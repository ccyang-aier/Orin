...
![图2：AIBrix 核心 CRD 的语义操作图](assets/02/02-crd-map.png)

图 2 不再把 CRD 画成简单对象清单，而是按“意图、AIBrix API、控制器工作、运行时输出”四列阅读。`ModelAdapter` 这一行展示 LoRA artifact 和 `podSelector` 如何进入 adapter 调度与加载，再落到 `Service`、`EndpointSlice`；`PodAutoscaler` 这一行展示指标来源和 `scaleTargetRef` 如何进入扩缩容决策；`StormService` 与 `RayClusterFleet` 两行展示复杂工作负载如何逐层下降到 Pod 或 RayCluster；`KVCache` 则展示缓存 mode、metadata 与 cache runtime 的管理关系。右侧的小状态块提醒读者：这些对象不仅有目标状态，也会持续产生 phase、ready、desired、actual 等可观测信号。
...
CRD 中心化容易被误解为“平台要重新实现 Kubernetes”。AIBrix 的实际设计恰好相反：CRD 表达领域意图，下游运行时资源执行具体行为。这里的运行时资源既包括 Kubernetes built-ins，例如 `Deployment`、`Service`、`Pod`、`EndpointSlice`、`HPA`；也包括 Gateway API 的 `HTTPRoute`、`ReferenceGrant`；还可能包括 KubeRay 的 `RayCluster` 这类可选扩展资源。

![图3：AIBrix CRD 与运行时资源的边界闭环](assets/02/03-crd-k8s-boundary.png)

图 3 把边界拆成三层。左侧是 domain intent：AIBrix CRD 的 `spec`、`status` 以及模型标签/注解表达平台语义。中间是 controller bridge：控制器通过 watch、reconcile 和 owner reference 把领域对象转成下游资源。右侧是 runtime surface：Kubernetes built-ins 承载 Pod、Service、HPA 等通用行为；Gateway API 承载 `HTTPRoute` 与跨 namespace 引用；KubeRay 在需要 Ray 分布式推理时承载 `RayCluster`。底部的反馈通道表示 metrics、adapter loaded、cache ready 等真实运行信号会回流到 CRD `status`。
...
控制器桥接两侧。它读取左侧的 CRD `spec`，创建或更新右侧的 Kubernetes、Gateway API、KubeRay 或运行时资源，再观察资源状态和运行时指标，最后回写左侧的 CRD `status`。这也是为什么 AIBrix 的 CRD 不应该被当成“部署样例集合”阅读：真正重要的是 `spec -> controller -> resource -> status` 这条闭环。
...
因此，读 AIBrix 架构时可以用一个判断规则：凡是表达 LLM Serving 平台意图、策略和状态的，通常属于 AIBrix CRD 或标签/注解语义；凡是负责实际创建容器、暴露网络、承载网关、维护端点或运行分布式集群的，通常属于 Kubernetes built-ins、Gateway API、KubeRay 或具体推理运行时。
...
![图4：AIBrix 控制器职责矩阵](assets/02/04-controller-ownership-loop.png)

图 4 用职责矩阵替代单链路示意图。`ModelRouter` 监听 `Deployment`、`ModelAdapter`、`RayClusterFleet` 等带模型身份的对象，并创建 `HTTPRoute`、`ReferenceGrant`，但它不是一个写回 CRD `status` 的控制器。`ModelAdapter controller` 负责 adapter 发现与服务发现资源；`PodAutoscaler controller` 负责指标读取和扩缩容目标；`StormService`、`RoleSet`、`PodSet` 形成工作负载编排链；`RayClusterFleet` 和 `RayClusterReplicaSet` 对接可选 KubeRay；`KVCache controller` 管理缓存 runtime 与 metadata service。

矩阵底部的 Kubernetes API as shared state 是理解控制器协作的重点。控制器之间不依赖直接函数调用来传递关键状态，而是通过 API 对象、watch、reconcile 和 owner reference 形成松耦合协作。
