...
![图2：AIBrix 核心 CRD 的语义分层图](assets/02/02-crd-map.png)

图 2 把 AIBrix CRD 压缩成三类语义来读。`Model surface` 关注模型服务入口和 adapter 暴露，典型对象是 `ModelAdapter`，运行时会落到 `Service` 和 `EndpointSlice`。`Workload shape` 关注复杂工作负载结构，`StormService`、`RoleSet`、`PodSet` 用于表达角色化 Pod 组织，`RayClusterFleet` 则面向 RayCluster 这类分布式推理实例。`Operations` 关注横向治理，`PodAutoscaler` 连接扩缩容目标，`KVCache` 连接缓存运行时。这样读图时，重点不再是背完所有对象，而是看清每组 CRD 在平台里负责哪一种语义。
...
![图4：控制器围绕 Kubernetes API 的协作方式](assets/02/04-controller-ownership-loop.png)

图 4 用四个职责岛替代密集矩阵。`Route bridge` 表示 `ModelRouter` 根据模型身份维护 `HTTPRoute`，但它不承担 CRD `status` 回写；`Adapter lifecycle` 表示 `ModelAdapter` 控制器围绕 adapter 生命周期维护 `Service`、`EndpointSlice` 和状态；`Workload controllers` 表示 `StormService`、`RoleSet`、`PodSet` 等工作负载对象通过 API 对象逐层协作，`RayClusterFleet` 则代表 Ray 工作负载入口；`Operations controllers` 表示 `PodAutoscaler` 和 `KVCache` 这类横向治理能力。

图中央的 Kubernetes API 是理解控制器协作的重点。控制器之间不依赖直接函数调用来传递关键状态，而是通过 API 对象、watch、reconcile 和必要的 owner reference 形成松耦合协作。
