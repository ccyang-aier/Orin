...
这里原始 token 数是 8，但 top-2 会产生 16 个 expert assignment。`Rank 0` 最终要处理 11 个 assignment，`Rank 1` 只处理 5 个。即使两个 rank 的 GPU 型号完全相同，`Rank 0` 也更可能成为 straggler；combine 阶段还要等待它的 expert 输出回来，再按 `t1...t8` 的原顺序和 gate weight 合并。所谓负载不均，真正坏在这里：静态 placement 看起来均匀，运行时 bucket 却可能被 router 拉歪。
...
不同框架对 EP group 的构造并不完全一样，下面这个 vLLM-style 的 8 GPU 例子只用于说明一类 serving 语义：服务层上，4 个 DP replica 各自内部用 TP=2 处理 attention；模型层上，MoE router 把 token 分到由 DP x TP 共同组成的 EP group，experts 分布在所有 8 张 GPU 上。也就是说，在这个示例里可以把 `EP_SIZE` 理解为 `DP_SIZE x TP_SIZE`，但不要把它泛化成所有框架的通用公式。
