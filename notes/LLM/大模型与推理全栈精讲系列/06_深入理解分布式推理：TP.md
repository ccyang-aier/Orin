...
很多人第一次真正遇到 TP，不是在论文里，而是在一次普通的模型部署里：模型文件已经下载好，机器也有多张 GPU，服务启动后却单卡 OOM，`nvidia-smi` 里其他 GPU 仍然空着。

这个现象反直觉的地方在于，**总显存够**并不等于**任意一张 GPU 都能放下一份完整模型副本**。只要模型副本必须完整放进一张 GPU，剩下的 GPU 再空也帮不上忙，于是问题从调参数变成了一个更底层的系统问题：一个 Transformer layer 能不能被多张 GPU 一起算？
...
大模型系统扩展通常有两条路：**复制**与**拆分**。

模型本身能放进一张 GPU 时，可以复制多份模型，让不同 GPU 处理不同 batch 或不同请求，这就是 Data Parallelism，也就是 DP。DP 很自然：
...
这些约束最终会落回两个基础矩阵模式：Column Parallel 与 Row Parallel。前者产生完整输出的一段，后者产生完整输出的一份贡献；MLP、Attention、Embedding 和 LM Head 的 TP 设计，都是在这两种语义上继续组合。
...
把 Column、Row、MLP、Attention 和 Vocab Parallel 放到同一张切分维度地图里，会更容易看清它们的共同结构：
...
分片机制解决的是数学等价问题：哪些张量能切，局部结果怎样恢复为完整层输出。通信机制回答的是成本问题：这些恢复动作要搬多少数据、走什么链路、是否会卡在关键路径上。
...
真正上线前，仍然要用真实 workload benchmark 验证 TTFT、TPOT、吞吐、显存水位和 NCCL 耗时；并行策略的名字不能替代测量结果。选择 TP size 时，不要只看 GPU 数，还要按四个问题判断：
...
TP 的核心不是一个命令行参数，而是一种看待 Transformer 层的方式。Transformer 层不是不可拆的黑盒，它由可切分的矩阵乘法、attention heads、embedding 和 projection 组成；TP 正是利用这些结构，把同一层内部的权重和计算拆到多张 GPU 上，再通过通信恢复数学等价性。

理解 TP 可以抓住四条主线：

1. **切分对象是层内张量**，不是请求队列，也不是普通模块搬家；
2. **Column Parallel 与 Row Parallel 的输出语义不同**，前者产生完整输出的一段，后者产生完整输出的一份贡献；
3. **通信是 TP 的成本中心**，attention output projection 和 MLP down projection 这类 Row Parallel 边界，常常会把 collective communication 放进每层关键路径；
4. **部署选择依赖模型、互联和 workload**，`tensor_parallel_size` 要同时匹配模型维度、显存压力、节点拓扑、KV cache 和实际 TTFT/TPOT/吞吐表现；

这些主线也能解释几个常见误判。TP 不等于多 GPU 一定更快：模型不大、batch 很小或互联较弱时，通信可能抵消并行收益。TP 也不等于把不同请求分给不同 GPU，那更接近 DP 或请求级负载均衡；在 TP 场景下，一个请求通常进入一个 TP group，由多张 GPU 共同完成一次 forward。

TP size 同样不是越大越好。规模变大后，每张卡的局部计算减少，但 collective communication 的参与设备也变多，跨慢链路时尤其容易把瓶颈推到通信上。TP 还不能自动解决所有显存问题，它主要分摊权重和层内计算，推理显存仍会被请求长度、并发数和 KV cache 强烈影响。
...
当你看到 `tensor_parallel_size`、`ColumnParallelLinear`、`RowParallelLinear`、`all-reduce`、`NVLink`、`TP + PP` 这些词时，可以把它们统一放回同一个问题：
...
错题回看建议：1-3 题回看第 1 章；4-9、15 题回看第 2 章；20 题回看第 3.2 节；10-12、16、18 题回看第 4 章；13、19 题回看第 4.1 节和第 5 章；14、17 题回看第 4.4 节。
