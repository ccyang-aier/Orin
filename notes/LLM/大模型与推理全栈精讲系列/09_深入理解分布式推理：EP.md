...
MoE 模型会带来第三类问题：模型看起来有很多参数，甚至总参数量远大于 dense 模型，可每个 token 并不会经过全部参数，而是只激活少数 experts。瓶颈不再只是**矩阵能不能被切开**或**请求能不能分给不同副本**，还多了一个更细的运行时问题：
...
这些现象只用 TP 或 DP 去解释还不完整。EP 出场的地方，正是 MoE 把**模型内部路由**变成系统问题的地方。
...
现代 MoE 里还经常要区分 **routed experts** 和 **shared experts**。Routed experts 是由 router 动态选择的专家，EP 主要讨论的就是这部分专家怎样分布、token 怎样 dispatch/combine。Shared experts 则更像每个 token 都会经过的共享 FFN 分支，是否复制、切分或与 routed experts 并行执行，要看具体模型和框架实现。默认语境下，experts 主要指 routed experts。
...
MoE 推理里还必须分清两层 router。服务层 router 处理用户请求，关心队列、KV cache、请求长度和副本负载，回答的是**这个请求交给哪个服务实例、哪个 DP replica、哪个 engine**；模型层 router 处理 token，关心 token-to-expert 映射、expert capacity、dispatch/combine 通信和 expert 负载，回答的是**这个 token 在当前 MoE layer 里交给哪些 experts**。两者粒度、状态和代价都不同，EP 讨论的是后者带来的系统问题。

## 2. 什么是 EP？

MoE 已经把 FFN 从一条固定路径变成了多条专家路径，EP 则进一步回答专家路径如何落到多张 GPU 或多个 rank 上。理解 EP 时，先把术语、定义和边界放在同一个视野里，避免把它误读成 TP、DP 或普通服务路由的变体。
...
这一定义要落到具体路径上：hidden states 先进入 Router / top-k，router 为每个 token 选择 experts；experts 分布在 GPU 0 到 GPU 3 上，token dispatch 把 token 送到对应 GPU；expert 计算完成后，combine 再把结果合回 dense path，让这个 dense path 继续进入下一层 Transformer。

EP 经常和 DP、TP、PP、CP/PCP/DCP 同时出现，但它们切的对象完全不同。
...
| CP / PCP / DCP | sequence/context、prefill 或 decode KV 相关切分 | 长上下文沿 token/上下文维度拆 | 主要服务 attention/KV，不决定 token 去哪个 expert |
| EP | MoE experts | experts 分布，token 找专家 | 只在 MoE expert 层有意义 |

看到 `EP enabled` 时，不要只问**用了几张 GPU**，更应该问：

**专家放在哪里？token 怎样过去？结果怎样回来？这条路径和 DP/TP/PP/CP/PCP/DCP 的边界在哪里？**
...
Router / gate 通常会产生 expert 选择，以及后续 combine 使用的权重。具体做法并不固定：有的模型使用 softmax，有的会用 sigmoid、top-k 后重新归一化、expert bias、group-limited routing 或 auxiliary-loss-free balancing。这里先抓住一个稳定语义：router 决定 token 去哪些 experts，并为这些 expert 输出提供合并权重。对 top-2 routing 来说，一个 token 可能同时去两个 experts，比如：
...
这里真正要保留的机制顺序是：先为每个 token 计算 expert score，再选择 top-k experts，随后按 expert 分桶 dispatch，本地 Expert FFN 计算，最后 combine 并恢复 token order。只要 combine 顺序或 gate weight 错了，token hidden state 就会串位，模型输出会直接失真。

## 4. 通信与负载

前面已经把 token 的路径拆成 router、dispatch、expert compute 和 combine。工程上真正棘手的是：这条路径不是固定形状的矩阵切分，而是由每一步的 token-to-expert 分布临时决定，通信量、bucket 大小和最慢 rank 都会随输入变化。理解 EP 的性能，需要同时看 all-to-all 搬了什么、负载为什么偏斜、capacity 如何处理过载，以及 prefill/decode 为什么呈现不同压力。

### 4.1 All-to-all

TP 里常见的通信心智模型是 all-reduce：多个 rank 计算同一个输出的不同贡献，然后求和并复制结果。EP 的通信模型更像 token exchange：每个 rank 有一些 token，要发给拥有对应 expert 的其他 rank；计算后，再把结果送回去。
...
如图，两条橙色通信带表示 dispatch 和 combine，真实系统里，rank 间 token 交换量取决于 router 的 token-to-expert 分布。某个 rank 如果收到特别多 token，其他 rank 即使本地计算结束，也可能要等待它完成。

需要注意，图里的橙色通信带表示逻辑 collective，不是一个中心化网关。真实 all-to-all 通常是多 rank 之间同时交换数据，只是图上用一条宽带来避免线条变成一团。

给一个数量级锚点：如果一次 MoE 层有 1024 个输入 token，hidden size 为 8192，BF16 每元素 2 bytes，top-2 routing 会形成大约

$$
1024 \times 2 \times 8192 \times 2 \approx 32 \text{ MiB}
$$

的逻辑 hidden-state dispatch payload。这里的 32 MiB 不是实际跨网字节数：本地 expert 命中不一定上链路，跨 rank / 跨节点传输量还会受到 expert locality、routing 分布、padding、metadata、backend 和网络拓扑影响。Combine 侧还要把 expert 输出送回并合并。这个估算不是精确性能模型，只是帮助建立直觉：EP 的通信搬运对象是 token hidden states，而不是一个小小的 expert id 列表。

EP 的性能不只看平均通信量，还要看最慢 rank：
...
用一个很小的 top-2 例子可以看清这个问题。假设有 4 个 experts，`E0/E1` 放在 `Rank 0`，`E2/E3` 放在 `Rank 1`。8 个 token 的路由结果如下：

| token | top-2 experts | dispatch 后的归属 |
| --- | --- | --- |
| t1 | E0, E1 | Rank 0, Rank 0 |
| t2 | E0, E2 | Rank 0, Rank 1 |
| t3 | E0, E1 | Rank 0, Rank 0 |
| t4 | E0, E3 | Rank 0, Rank 1 |
| t5 | E0, E1 | Rank 0, Rank 0 |
| t6 | E0, E2 | Rank 0, Rank 1 |
| t7 | E1, E3 | Rank 0, Rank 1 |
| t8 | E0, E1 | Rank 0, Rank 0 |

这里原始 token 数是 8，但 top-2 会产生 16 个 expert assignment。`Rank 0` 最终要处理 11 个 assignment，`Rank 1` 只处理 5 个。即使两个 rank 的 GPU 型号完全相同，`Rank 0` 也更可能成为 straggler；combine 阶段还要等待它的 expert 输出回来，再按 `t1...t8` 的原顺序和 gate weight 合并。落到监控上，常见信号是 token-per-expert histogram 向少数 experts 倾斜、per-rank wait time 拉开、all-to-all p99 比平均值更刺眼。所谓负载不均，真正坏在这里：静态 placement 看起来均匀，运行时 bucket 却可能被 router 拉歪。

这也是为什么 MoE 训练里常见 auxiliary loss、capacity factor、token dropping/dropless routing、expert-wise bias 等负载均衡或容量/过载处理机制。推理系统里仍然可能需要运行时 EPLB（Expert Parallel Load Balancer）或类似策略。这里要特别区分：dropless 表示尽量不丢 token，它本身不自动保证负载均衡，反而会把热门 expert 的压力真实暴露给通信、buffer 和尾延迟。

### 4.3 Capacity 与 token dropping

MoE 里常见的 capacity 概念，用来限制每个 expert 在一个 batch 或 micro-batch 中接收多少 token。一个粗略直觉是：如果共有 $T$ 个 token、每个 token 选择 top-k experts、共有 $E$ 个 experts，那么平均每个 expert 会收到大约 $T \times k / E$ 个 assignment；capacity factor 会在这个平均值上留出余量，避免轻微偏斜就把 expert 打满。

问题在于 router 的分布很少刚好均匀。热门 expert 一旦超过 capacity，系统必须在几类策略之间取舍：

| 策略 | 常见位置 | 代价 |
| --- | --- | --- |
| token dropping | 训练中更常见 | 牺牲一部分 token 的 expert 计算路径，可能影响质量，需要训练目标配合 |
| capacity padding | 训练和推理都可能出现 | bucket 形状更规整，但会引入无效计算和额外 buffer |
| dropless routing | 在线推理更常见 | 保留 token 质量路径，但把热点压力转化为通信、计算和尾延迟压力 |
| placement / redundant expert | 推理服务优化 | 通过调整 expert 映射或复制热点 expert 缓解过载，但消耗显存并增加调参复杂度 |

**router 的选择不是免费且无限的。热门 expert 会消耗更多计算、通信和 buffer；系统必须决定如何处理过载。**

在线推理通常不能随意丢 token，因为这会直接影响输出质量和可重复性。于是推理侧更常见的压力是：不丢 token，但要承担更不均匀的 bucket、更高的尾延迟，或者通过 expert placement / redundant expert / EPLB 来缓解。换句话说，capacity 不是一个单纯的训练超参数，它也会影响 serving 系统如何设计 buffer、collective 波次和负载均衡策略。

### 4.4 Prefill 与 Decode 的压力不同

LLM 推理里，prefill 和 decode 对 EP 的压力也不一样。

Prefill 阶段一次处理 prompt 中的大量 token。token 多，expert bucket 往往更容易攒起来，GPU 计算利用率更容易做高，但 all-to-all 的总通信量也会更大。
...
因此，同一套 EP 配置不能只用一个平均延迟来判断。Prefill 更像一次大批量搬运和计算，常看 TTFT、吞吐、网络带宽和显存峰值；decode 更像很多次小步同步，常看 TPOT、p50/p99 all-to-all、per-rank wait time 和是否存在少数 hot ranks。把 prefill 与 decode 混在同一组指标里，很容易看漏**吞吐变好但逐 token 尾延迟变差**的情况。
...
vLLM 的 EPLB 就是一个面向推理服务的例子：系统在 forward 中收集负载统计，按窗口和间隔周期性重平衡 expert mapping；也可以配置 redundant experts，让热门专家在更多 rank 上可用。这样做的目标不是重新训练 router，而是在 serving/runtime 层面降低热点和 straggler。需要注意，vLLM Expert Parallel Deployment 文档中的 `--eplb-config`、all-to-all backend 和 expert placement strategy 等参数属于框架实现语义，随版本演进时要以对应版本官方文档为准。

![热点专家与负载均衡|900](imgs/ep-hot-expert-placement-handdrawn-cn.png)

这里真正要保留的判断是：运行时可以通过调整 placement 或复制热点 expert 缓解负载，但这不表示所有 experts 都应该被全量复制到每张 GPU。

不过，冗余 expert 有明确成本。它会增加显存占用，可能挤压 KV cache 空间，也会引入 expert 权重迁移、统计窗口和重平衡频率的调参问题。因此，启用 EPLB 前要先确认瓶颈真的是 expert hotspot，而不是 attention、KV cache、API server 或网络出口。

## 5. EP 与 DP、TP、PP、CP 的组合

理解了 EP 层内 token 如何移动之后，还要追问参与通信的 ranks 来自哪种并行拓扑。`DP=4, TP=2, EP enabled` 不是一个简单乘法题，它描述的是同一批 GPU 在不同层里的多重身份：服务层可能按 DP 分请求，attention/dense 层可能按 TP 切矩阵，MoE expert 层又可能把这些 ranks 组织成 EP collective。阅读这类配置时，核心不是记住缩写组合，而是把**请求路径、dense 计算路径、expert 路由路径**分开。
...
在 vLLM 类需要所有 EP ranks 保持 collective wave 一致的 serving 实现中，如果一个 rank 在当前 step 没有本地 ready request，但它属于 MoE collective 的参与集合，它仍可能需要执行 dummy forward 或参与协调。否则 collective 调用顺序和参与 rank 不一致，就可能 hang。这个判断不能机械外推到所有 EP 系统，关键要看该框架如何定义 collective 参与集合和调度波次。
...
- PP 解决层太多、跨节点按层放置更自然的问题；
- EP 解决某个 MoE layer 里的 experts 太多或专家计算太重的问题；
...
### 5.4 CP / PCP / DCP 与 EP：不要把长上下文切分当 expert routing

CP、PCP 或 DCP 主要处理长上下文带来的 sequence/context 和 KV cache 压力。它们关心的是 token 序列怎样分块、attention 如何看到全局 K/V、prefill 或 decode 阶段怎样减少 KV 重复或局部压力。CP/PCP/DCP 会在后续单篇展开；这里先把它理解成面向长上下文和 KV cache 的切分即可。以 vLLM 语境为例，DCP 指 Decode Context Parallelism，它复用 TP group，不增加 worker 数，目标之一是减少 decode 阶段 KV cache 在 TP ranks 上的重复；这和更一般的 context parallelism 或 prefill context parallelism 不应混为同一个实现细节。

EP 关心的是 token 被 router 送到哪些 experts。两者都可能让 token 维度发生重排，但问题不同：

- CP/PCP/DCP：为 attention 和 KV cache 服务；
- EP：为 MoE expert FFN 服务；

长上下文 MoE 服务可能同时需要 CP/PCP/DCP 和 EP。此时更要分清：一个 token 的上下文分片位置，不等于它在 MoE layer 里选择的 expert。

### 5.5 一个组合例子

不同框架对 EP group 的构造并不完全一样，下面这个 vLLM-style 的 8 GPU 例子只用于说明一类 serving 语义：服务层上，4 个 DP replica 各自内部用 TP=2 处理 attention；模型层上，MoE router 把 token 分到由 DP x TP 共同组成的 EP group，experts 分布在所有 8 张 GPU 上。也就是说，在这个示例里可以把 `EP_SIZE` 理解为 `DP_SIZE x TP_SIZE`。这是 vLLM 当前 serving 文档中的自动计算方式，不是 TensorRT-LLM、SGLang、Megatron 或 DeepSpeed 的通用公式。

![EP 与 DP、TP 的组合|900](imgs/ep-dp-tp-composition-handdrawn-cn.png)

这个例子最重要的是边界：蓝色的服务/attention 路径和紫色的 token 路由不是同一层路由。请求先被服务层分配，token 在 MoE layer 内又被模型层 router 分配。只有把这两层分开，才能理解为什么 MoE 部署里的 DP + EP 比 dense 模型 DP 更复杂。
...
**如果瓶颈来自 MoE experts 的权重、计算或分布，EP 值得看；如果瓶颈来自普通 dense layer、请求吞吐或长上下文 KV cache，先看 TP、DP 或 CP/PCP/DCP。**
...
| KV cache OOM | EP 不会自动消除长上下文 KV 状态 | CP/PCP/DCP、KV cache 配置、并发限制 |
...
| 系统 | 常见入口或概念 | 心智模型 |
| --- | --- | --- |
| vLLM | `--enable-expert-parallel`、`--all2all-backend`、`--expert-placement-strategy linear/round_robin`、`--eplb-config` | 在 vLLM serving 文档语义下，`EP_SIZE = TP_SIZE x DP_SIZE`，MoE expert 层可在这些 ranks 上分布；EPLB 用于运行时 expert 负载均衡 |
| Megatron Core / Megatron-LM | `--num-experts`、`--expert-model-parallel-size`、`--moe-router-topk`、`--moe-token-dispatcher-type alltoall/flex/allgather`、`--moe-flex-dispatcher-backend deepep/hybridep`、`--moe-grouped-gemm` | router、token dispatcher、GroupedGEMM、DeepEP/HybridEP 等组合成 MoE 执行路径，EP/TP/hybrid 由训练或推理配置显式决定 |
...
| TensorRT-LLM | `moe_tensor_parallel_size`、`moe_expert_parallel_size`、hybrid MoE sharding、Wide-EP / expert slots | 区分 TP 对 expert 权重的切分与 EP 对 experts 的分布，`moe_tensor_parallel_size x moe_expert_parallel_size = tensor_parallel_size`；较新的材料还强调 Wide-EP、专家槽位和负载均衡 |
| SGLang | `--tp`、`--ep` / `--ep-size`、`--moe-a2a-backend`、`--moe-runner-backend`、`--deepep-mode`、`--enable-eplb` | TopK routing 之后执行 dispatch，交给 grouped GEMM/MoE runner，再 combine；EP、all-to-all backend、MoE runner 和 EPLB 是需要一起调的执行路径 |
...
11. collective 参与集合和调用顺序是否稳定，框架是否需要 dummy forward、coordinator 或其他波次协调？
12. 真实 workload 下 TTFT、TPOT、throughput、all-to-all 时间是否真的改善？

## 7. 本章小结

EP 的主线可以从三个层次收束：它切分什么、token 如何流动、系统为什么会因为负载尾部变慢。只要这三层没有混在一起，看到不同框架的 EP 参数时，就能把配置还原成可验证的工程问题。

### 7.1 边界：EP 只处理 MoE expert 层

EP 的切分对象是 MoE experts，不是普通 dense layer 的矩阵，也不是用户请求。TP 关注 dense 层内矩阵或张量维度，DP 关注副本和请求分流，PP 关注层段放置，CP/PCP/DCP 关注长上下文和 KV cache 压力；EP 只在 MoE expert 层有直接意义。

这个边界很重要，因为 MoE 模型里仍然有 attention、embedding、shared experts、KV cache、activation、workspace、communication buffer 和 adapter 状态。开启 EP 可以分布 routed experts，但不能自动解决所有显存、吞吐或长上下文问题。部署判断要从瓶颈出发，而不是从是否能打开某个 EP 开关出发。

### 7.2 机制：token 经过一次可逆的专家旅程

EP 的运行核心是 token 旅程：router 为每个 token 选择 experts，dispatch 按 expert/rank 重排并发送 token，本地 Expert FFN 处理收到的 bucket，combine 再按原 token 顺序和 gate weight 合并结果。

这条路径必须保持可逆。Dispatch 可以打乱 token 顺序，top-k 可以让同一个 token 产生多个 expert 输出，但 combine 必须把结果放回正确 token 位置，并按 router 权重合并。只要 expert mapping、combine 顺序或 gate weight 出错，问题就不再只是性能下降，而是模型输出语义被破坏。

### 7.3 成本：动态负载决定尾部体验

EP 的性能核心不是平均 expert 数量，而是运行时 token-to-expert 分布。静态 placement 可以让每张 GPU 拿到相同数量的 experts，但 router 仍可能把 token 集中送向少数 hot experts；这些 experts 所在 rank 会承担更大的 bucket、更长的 GEMM、更大的通信消息和更高的 buffer 压力。

因此，评估 EP 要看最慢 rank、all-to-all p99、token-per-expert histogram、per-rank wait time、prefill/decode 的差异，而不是只看平均吞吐。Prefill 往往更像大批量 token 搬运和计算，decode 更容易暴露小 bucket、collective 等待和逐 token 尾延迟。Capacity、dropless 推理、EPLB 和 redundant experts 都是在处理同一个底层事实：router 的选择会把负载真实地投射到通信、计算和显存上。

### 7.4 判断：把 EP 放回完整并行拓扑

真实 MoE 推理里，EP 很少孤立存在。一个部署可以同时有 DP 的请求分流、TP 的 dense 层切分、PP 的层段放置、CP/PCP/DCP 的长上下文优化，以及 EP 的专家分布。阅读 `DP=4, TP=2, EP enabled` 这类配置时，不要只把数字相乘，而要拆成三条路径：请求如何进入 replica，attention/dense 层如何执行，MoE expert 层如何形成 collective。

最终要追问的是一组可观测问题：**expert 放在哪里？token 怎样过去又怎样回来？负载是否均衡？collective 参与集合是否稳定？EP 和 DP/TP/PP/CP/PCP/DCP 的边界在哪里？真实 workload 下 TTFT、TPOT、throughput 和 all-to-all 尾延迟是否同时改善？**
...
7. [TensorRT-LLM Documentation: Parallel Strategy](https://nvidia.github.io/TensorRT-LLM/latest/features/parallel-strategy.html)；
8. [SGLang Documentation: Expert Parallelism](https://sgl-project.github.io/advanced_features/expert_parallelism.html)；
9. [GShard: Scaling Giant Models with Conditional Computation and Automatic Sharding](https://arxiv.org/abs/2006.16668)；
...
13. [DeepSeek-V3 Technical Report](https://arxiv.org/abs/2412.19437)；
14. [Auxiliary-Loss-Free Load Balancing Strategy for Mixture-of-Experts](https://arxiv.org/abs/2408.15664)；
15. [DeepSeek EPLB](https://github.com/deepseek-ai/EPLB)；
16. [DeepEP](https://github.com/deepseek-ai/DeepEP)；
17. [NVIDIA NCCL User Guide: Collective Operations](https://docs.nvidia.com/deeplearning/nccl/user-guide/docs/usage/collectives.html)；
...
3. 多选：一次典型 MoE EP forward 的 token 流包括哪些步骤？
   ...
   E. 用 optimizer state 更新 expert 权重；
...
9. 单选：EP 与 CP/PCP/DCP 的区别是什么？
   A. EP 分布 experts，CP/PCP/DCP 处理长上下文或 KV/cache 相关的序列维切分；
   B. EP 只切 KV cache；
...
20. 场景题：decode 阶段 all-to-all p99 延迟抖动明显，但平均 token-per-expert 看起来还可以，应该优先继续排查哪些方向？至少答出 4 类可验证方向。
...
3. A、B、C、D。EP 的关键不是每张卡算同一批 token，而是 token 被动态分发给拥有对应 experts 的 rank，再被合回原 token 顺序。E 是训练期权重更新语义，不属于推理 forward 的 token 流；
...
9. A。EP 面向 MoE FFN/expert 层；CP/PCP/DCP 面向上下文长度、prefill/decode 和 KV 访问压力；
...
20. 应优先排查 p99 而不是只看平均值。答出其中任意 4 类并能说明验证指标即可：是否存在少数 hot experts 或 hot ranks；decode 并发是否太低导致 bucket 过小；all-to-all 是否跨节点或跨 NUMA 弱链路；通信 backend 是否适配当前拓扑；是否有 collective wave 等待、dummy forward 或 rank 空转；prefill 与 decode 是否混在同一指标里；EPLB 的统计窗口和重平衡是否引入抖动；
