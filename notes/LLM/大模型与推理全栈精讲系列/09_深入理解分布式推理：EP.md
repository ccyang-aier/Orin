...
这一定义要落到一个具体路径上：hidden states 先进入 Router / top-k，router 为每个 token 选择 experts。Experts 分布在 GPU 0 到 GPU 3 上，token dispatch 会把 token 送到对应 GPU；expert 计算完成后，combine 再把结果合回 dense path。这个 dense path 可以继续进入下一层 Transformer。
...
这也是为什么 MoE 训练里常见 auxiliary loss、capacity factor、token dropping/dropless routing、expert-wise bias 等负载均衡或容量/过载处理机制；而推理系统里仍然可能需要运行时 EPLB（Expert Parallel Load Balancer）或类似策略。这里要特别区分：dropless 表示尽量不丢 token，它本身不自动保证负载均衡，反而会把热门 expert 的压力真实暴露给通信、buffer 和尾延迟。

### 4.3 Capacity 与 token dropping

【批注，4.3内容太少，看能不能合并到其它小节，或者把这个小节再全面铺开说明下】
...
因此，同一套 EP 配置不能只用一个平均延迟来判断。Prefill 更像一次大批量搬运和计算，常看 TTFT、吞吐、网络带宽和显存峰值；decode 更像很多次小步同步，常看 TPOT、p50/p99 all-to-all、per-rank wait time 和是否存在少数 hot ranks。把 prefill 与 decode 混在同一组指标里，很容易把“吞吐变好但逐 token 尾延迟变差”的情况看漏。
...
【批注，重构下面这段话，类似真正的难点xxx很莫名其妙，数字相乘还是是难点吗？逻辑都不通】

理解了 EP 层内 token 如何移动之后，还要追问这些参与通信的 ranks 到底来自哪种并行拓扑。看到 `DP=4, TP=2, EP enabled` 这类配置时，真正的难点不是把数字相乘，而是分清同一批 GPU 在不同层里承担的角色：服务层可能按 DP 分请求，attention/dense 层可能按 TP 切矩阵，MoE expert 层又可能把这些 ranks 组织成 EP collective。下面几节按这个场景拆开，而不是把每个缩写当作孤立参数。
...
【批注，本小节标题改为本章小结，重构内容，不要说常见误区和心智模型，对本章做一个阶段性小结，条理清晰循序渐进，内容和见解有深度】
...
第四，EP 很少孤立存在。真实 MoE 推理通常还要同时判断 DP 的请求分流、TP 的 dense 层切分、PP 的层段放置、CP/PCP/DCP 的长上下文压力。

所以，看到 EP 时不要只问“开没开专家并行”，而要问：

**expert 放在哪里？token 怎样过去又怎样回来？负载是否均衡？EP 和 DP/TP/PP/CP/PCP/DCP 的边界在哪里？**
