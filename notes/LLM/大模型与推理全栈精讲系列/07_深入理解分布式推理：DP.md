...
# Data Parallelism（DP）是什么

> [!Quote] 本篇导读
> DP（Data Parallelism，数据并行）最朴素的含义不是“把一张 GPU 变大”，而是“复制可独立运行的模型执行单元，让不同数据或不同请求分头处理”。如果说 TP 的关键词是把同一个 layer 切开共同计算，那么 DP 的关键词就是副本、分流、容量和状态边界。
...
【批注，你都没铺垫router，怎么突然来了一个Router 做了什么？？】
...
### 2.2 KV cache 为什么让推理 DP 更像系统问题【批注，改名字，不要总拿结论或者问题当标题】
...
【批注，4、5节合并，另外注意小节标题不要太长，不要总是拿问题或一个还未引入或铺垫分析的结论作为标题】
...
【批注，7、8节合并，作为本章小结，重构第七节内容，不要说什么常见误区】
