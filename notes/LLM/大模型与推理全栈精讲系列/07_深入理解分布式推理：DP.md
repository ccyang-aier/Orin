...
---
# Data Parallelism（DP）核心模型

> [!Quote] 本篇导读
> DP（Data Parallelism，数据并行）最朴素的含义不是把一张 GPU 变大，而是复制可独立运行的模型执行单元，让不同数据或不同请求分头处理。如果说 TP 的关键词是把同一个 layer 切开共同计算，那么 DP 的关键词就是副本、分流、容量和状态边界。
...
这句话没有写成每张 GPU 都有完整模型，原因是现代系统里，一个 DP 副本不一定等于一张 GPU。它可以是：
...
- DP 关心的是有多少份可独立运行的模型执行单元；
- TP 关心的是同一个模型层内部的张量和计算如何拆到多张 GPU；
...
把 DP 理解成副本之后，推理服务的主线就变成了**请求怎样进入副本**。一个请求不会凭空落到某张卡上，它通常先经过 API server、gateway、scheduler 或外部 load balancer；这些组件可以合在一个进程里，也可以分布在多个服务里，但它们共同承担了选择副本的职责，教程里统一称为 router。

### 2.1 路由与副本选择

在推理服务里，DP 最常见的形态是请求级分流。用户请求先进入服务入口，再被分配给某个 DP replica；分配策略越了解副本的真实状态，DP 越可能接近预期扩容效果。

一个能力较强的 router 通常会关心：
...
vLLM 的 data parallel deployment 文档把几种形态分得很清楚：内部负载均衡可以用一个 API endpoint 暴露多个 DP ranks；DP 也可以与 TP 组合，例如 `--data-parallel-size 4 --tensor-parallel-size 2` 表示 4 个 DP 副本，每个副本内部用 2 张 GPU 做 TP，总共需要 8 张 GPU。文档还提醒 `--max-num-seqs` 是按 DP rank 生效，而不是全局共享一个值。

```bash
vllm serve $MODEL --data-parallel-size 4 --tensor-parallel-size 2
```

这条命令背后的心智模型不是 8 张 GPU 一起处理同一个请求，而是有 4 个可独立接请求的副本，每个副本内部由 2 张 GPU 共同运行一个模型执行单元。

这里说的是概念上一个理想 router 需要哪些信息。vLLM 文档中的内部 DP load balancing 主要基于各 engine 的 running queue 和 waiting queue；KV cache aware logic 属于文档中提到的未来可增强方向。多节点部署时，还要明确 local DP size、start rank、headless 节点、hybrid LB 或外部 LB 的模式，不要只看 `data_parallel_size` 这一个参数。

![推理 DP 的请求路由与 DP x TP 组合|900](imgs/dp-inference-routing-cn.png)

### 2.2 KV cache 边界

请求被路由到副本以后，真正影响推理 DP 的不是梯度同步，而是运行时状态。LLM 推理里最重要的运行时状态之一是 KV cache。

在一次生成过程中，prefill 会根据 prompt 建立历史 token 的 key/value 状态，decode 会不断读取并追加这些状态。这个状态通常留在处理该请求的 engine 或 replica 内部，于是推理 DP 有一个很重要的边界条件：

**同一次正在生成的请求不能被随意拆到另一个副本继续 decode，除非系统显式支持状态迁移或重新计算。**

这里要区分聊天应用里的多轮对话和服务端持久状态。如果每次 API 调用都把完整历史上下文重新发给模型，那么新的请求可以重新路由；如果系统做了服务端会话级 KV cache、prefix cache 或跨请求状态复用，就需要会话粘性、cache 命中策略或状态迁移机制。
...
路由解决请求进入哪个副本，容量问题则决定一个副本能不能接住这些请求。DP 很容易给人一种错觉：既然有 4 张 GPU，总显存是不是就变成 4 倍了？

对推理 DP 来说，答案通常是否定的。DP 增加的是副本数，不会把一个副本的模型权重自动铺到多张卡上；每个副本仍然需要保存自己运行所需的权重、KV cache、workspace、通信 buffer 和调度状态。
...
有了路由和容量边界，扩容就不再只是把 GPU 数填进参数。一个更稳定的推演方式是先确认单副本，再增加副本，最后组合模型并行。
...
第三步，`DP=4, TP=2`。如果单副本需要 2 张 GPU 才能跑，TP 先在副本内部解决一个执行单元如何成立，DP 再在副本之间解决更多请求如何分流。此时总共通常需要 8 张 GPU，但不是 8 张 GPU 共同服务一个请求，而是 4 个双卡副本共同服务不同请求。

观察这类扩容时，不要只看总吞吐，还要同时看 TTFT、TPOT、每个副本的队列长度、KV cache 使用率、请求长度分布和 router 是否形成热点。DP 的价值来自有效副本数，而不是配置里写了多少副本。

## 3. 并行策略的位置

推理 DP 解释的是副本和请求之间的关系，但一个副本内部可能还要解决模型放置、层内计算或 MoE expert 路由。把这些策略放在同一张地图里看，DP 的边界会更清楚：它通常负责副本之间的工作分流，而不是替代 TP、PP、EP 或训练状态分片。
...
当一个模型副本需要两张或多张 GPU 才能跑起来时，TP 负责副本内部的模型执行，DP 负责副本之间的请求分流。
...
Data Parallelism 在训练里也很常见，但它不是把推理路由搬到训练系统里。训练 DP 和推理 DP 的共同动作都是复制模型执行单元、切分不同数据；区别在同步边界。
...
第 $r$ 个 rank 只看自己的 batch shard，得到本地梯度 $g_r$；所有 rank 通过 all-reduce 得到聚合结果，并由框架按 world size 做平均或等价归一化，得到一致的用于更新的梯度 $g$；然后每个副本用同样的梯度更新参数。只要初始参数一致、梯度同步一致、optimizer step 一致，副本就能保持一致。
...
理解 all-reduce 时，要把 collective primitive 和训练语义分开。NCCL 对 all-reduce 的定义是：每个 rank 提供输入，collective 对这些输入做 reduce，再把相同的 reduce 结果放回每个 rank。这里的 reduce 可以是 sum、min、max 等操作；平均梯度是 DDP 训练语义和缩放策略的一部分，不是 NCCL all-reduce 这个 primitive 自带的唯一含义。
...
NCCL 文档还强调，collective operation 需要每个 rank 以相同 count 和 datatype 参与，否则可能出现 hang、crash 或数据损坏。实际系统还必须保证参与集合、collective 类型和调用顺序一致，这解释了很多分布式训练错误为什么看起来像某张卡卡住了：真正的问题可能是某个 rank 跳过了一个 collective，或者进入 collective 的张量形状与其他 rank 不一致。

训练 DP 也不一定等整个 backward 完全结束后才同步所有梯度。PyTorch DDP 会把参数放入多个 bucket；某个 bucket 的梯度准备好后，就可以开始对应的 gradient reduction，从而让通信尽可能与后续 backward 计算重叠。`bucket_cap_mb` 这类参数影响的是通信颗粒度和重叠机会，不改变 DP 的数学语义。
...
如果每个 rank 都复制这些状态，DP size 越大，集群总显存消耗越重复。ZeRO 和 FSDP 要解决的就是这类训练状态冗余。
...
从状态布局看，它们不再是经典完整复制；从训练语义看，它们仍然围绕不同 rank 处理不同数据，并协同完成等价更新展开。因此更准确的说法是：**ZeRO/FSDP 是 sharded data parallelism，而不是普通推理 DP 的直接替代品。**

如果目标是理解 LLM 服务扩容，ZeRO/FSDP 不应该喧宾夺主。它们主要面向训练状态冗余与超大模型训练显存，而不是在线推理请求分流。当然，训练和推理会在一个完整平台里同时出现，例如一个团队可能用 FSDP 训练模型，再用 vLLM 做 DP x TP 推理部署；学习 DP 时先把两条线分开，就不容易把服务里多开副本和训练里切 optimizer state 混成同一个问题。
...
工程上判断是否该用 DP，先看单个可执行副本是否已经成立。如果副本本身能稳定加载模型、完成请求，并且主要瓶颈是更多独立请求或样本需要处理，那么 DP 往往是自然的扩容方式。
...
在线推理里，DP 常见于单模型多副本服务、多租户请求分流、离线评测或批量生成、单节点多副本部署、多节点外部负载均衡，以及 DP x TP 组合部署。
...
**如果瓶颈是更多独立工作要处理，DP 很合适；如果瓶颈是一个工作单元本身太重，通常要先看 TP、PP、量化或模型结构优化。**
...
- 普通推理 DP 不会把多张 GPU 显存合并成一个共享池；
- DP size 变大不保证吞吐线性增长，真实收益取决于有效副本数；
- 推理 DP 和训练 DP 共享复制加切分的思想，但同步机制不同；
- TP 解决副本内部的问题，DP 解决副本之间的问题，两者经常组合；
- ZeRO/FSDP 属于训练状态分片，不是推理 DP 的高级版；

所以，看到一个系统写着 `DP enabled` 时，不要只问用了几张 GPU，而要追问：
...
14. 多选：部署 DP 前，哪些检查更有价值？
    ...
    C. 长请求是否会造成负载不均；
    D. 扩大 DP size 后，同时观察 TTFT、TPOT、throughput 和分副本指标；
...
错题回看建议：1-3 题回看第 1 章；4-8 题回看第 2 章和第 5 章；9-10 题回看第 4 章；11 题回看第 5 章；12-13 题回看第 3 章；14-15 题回看第 5 章和第 6 章。
...
14. A、B、C、D。DP 之前要确认副本边界、并发参数语义和负载不均风险；扩大 DP size 后，也要同时观察 TTFT、TPOT、throughput 和分副本指标，才能判断扩容是否健康；
