...
>
> 这一篇我们先从最典型、也最重要的一类算力卡，GPU讲起。我们不会急着罗列各种术语，而是从CPU和GPU的差异出发，再通过一个贯穿全文的案例，把GPU的存储层次、计算单元、执行模型和性能瓶颈串起来。理解了这些基本原理后，后面再看到不同型号的卡、不同任务的性能表现，乃至于进一步学习推理相关知识点时，就会更容易建立自己的判断。
...
到了大模型时代，这种差异被进一步放大。模型推理和训练都不是偶尔做一次小计算，而是反复在处理大批量张量：一批token会变成一批向量，一层层权重会参与矩阵运算，中间结果还要继续传给下一层，循环往复。在这个过程里，最常见的底层工作形态正是大量位置上重复执行相似的数值计算。

这正是GPU所擅长的，也就是说，大模型会大量依赖GPU，并不是因为GPU对模型语义有特殊理解，而是因为矩阵乘、向量加法、归一化、激活函数等操作都能被拆成许多形状相似的小任务。只要任务规模足够大、结构足够规则，GPU的高吞吐优势就有机会释放出来，而大模型的训推效率和发展也得以借助GPU的算力优势而得到飞速发展。

### 1.2 用矩阵乘看并行性

上一小节说 GPU 喜欢海量相似任务，但这句话还比较抽象。下面我们看一个大模型里反复出现的计算：`Linear`层。暂时不需要理解它在神经网络里的全部语义，只需要知道它做了一件事：把一批输入向量乘上一块权重矩阵，得到一批新的输出向量。
...
上一节我们只是基于一个简单的案例来说明GPU的工作模式，即把大量的重复性工作拆开并行执行。然而，拆开以后还有一个更实际的问题：每个小任务开始计算之前，都要先拿到自己需要的数据，计算单元再多，如果数据迟迟送不到，它们也只能等。
...
| 层级                  | 直觉                 | 在 LLM 中的角色                |
| ------------------- | ------------------ | ------------------------- |
| HBM / Global Memory | 容量大、带宽高，但离计算单元远    | 存放权重、激活、KV cache、临时buffer |
| L2 Cache            | L2缓存，所有计算单元共享      | 多个计算区域访问共享数据时的重要缓冲层       |
