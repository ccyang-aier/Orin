...
> [!Quote] 本篇导读
>
> 在学习大模型推理的时候，我们经常会遇到一些让人困惑的现象：同一个模型，明明batch size翻倍了但吞吐却没跟着翻倍；一批请求的prefill很快，decode却拖得很慢；换了张更好的卡，某些场景的提升却仍然不及预期等等。这些现象初看可能让我们一头雾水，但一切令人难解的问题背后，真相往往有迹可循。
>
> 当我们在和大模型打交道的时候，大模型自身却正在和更底层的硬件打交道。以一次简单的对话为例：你输入一段提示词，模型需要把它转成数字、反复做大量矩阵运算、再把结果解码回文字，每生成一个token，这个过程就要完整走一遍。而在背后默默支撑这一切的，正是GPU。它不是因为更聪明才被选中，而是因为它天生擅长把海量形状相似的计算任务同时铺开，恰好契合大模型推理的需求。理解GPU是怎么工作的，是读懂后面一切性能问题的基础。
>
> 我们从一个问题开始：CPU和 GPU 为什么不是同一种机器？搞清楚这一点之后，再引入一个贯穿全篇的计算示例，把存储、计算单元、执行模型和性能瓶颈一层层串起来。
...
到了大模型时代，这种差异被进一步放大。模型推理和训练都不是偶尔做一次小计算，而是反复处理大批量张量：一批 token会变成一批向量，一层层权重会参与矩阵运算，中间结果还要继续传给下一层。这里最常见的工作形态，正是大量位置上重复执行相似的数值计算。

因此，大模型会大量依赖 GPU，并不是因为 GPU 对模型语义有特殊理解，而是因为矩阵乘、向量加法、归一化、激活函数等操作都能被拆成许多形状相似的小任务。只要任务规模足够大、结构足够规则，GPU 的高吞吐优势就有机会释放出来。

### 1.2 用矩阵乘看并行性

上一小节说 GPU 喜欢海量相似任务，但这句话还比较抽象。下面看一个大模型里反复出现的计算：`Linear` 层。它做了一件事：把一批输入向量乘上一块权重矩阵，得到一批新的输出向量。
...
其中 $B$ 表示一次处理多少组输入，$seq$ 表示一次处理多少个位置，$d_{model}$ 和 $d_{out}$ 表示每个位置上的向量宽度。
...
这自然引出下一个问题：每个 tile 需要的数据从哪里来，数据到达后由哪些硬件单元计算，软件又如何把整片输出空间交给 GPU 执行。
...
这四层负责存放或临时缓存数据；真正执行乘法、加法和矩阵乘累加的硬件单元是另一回事，下一节再介绍。也就是说，一次计算能不能顺利展开，不只取决于”有没有足够的计算单元”，还取决于数据能不能以合适的节奏来到这些计算单元附近。
...
到这里，$Y=XW$ 多了一层含义：它不只是一组可以并行的输出位置，还是一场数据搬运与复用的组织问题。接下来用同一块权重矩阵看一个更贴近推理的现象：为什么 prefill 和 decode 的数据压力并不一样。
...
上面的例子说明了直觉：prefill 和 decode 面对的数据压力不同。接下来用硬件峰值数字把这个差异变得更具体。

以 NVIDIA A100 80GB SXM 为例，dense FP16 矩阵计算峰值约为 312 TFLOPS；HBM2e 带宽约为 2,039 GB/s。A100 80GB PCIe 版本的带宽则约为 1,935 GB/s；如果使用结构化稀疏路径，标称峰值可到 624 TFLOPS。这里只用这些数字建立量级感，不同产品形态、dense/sparse 路径和实际实现性能之间存在差异，不宜混成一个数字。

这两个数字的量纲不同：算力单位是 FLOPS，带宽单位是 Bytes/s。把它们放在一起，自然会问：每搬运一个 byte 的数据，GPU 最多能做多少次浮点运算？这个比值就是 Arithmetic Intensity（计算强度）：
...
这里的数字是硬件峰值，不等于任意 PyTorch 代码都能达到。它们真正的作用是告诉我们：如果一个算子每从显存读入很少数据就能做大量计算，它更有机会接近计算上限；如果一个算子读写了大量数据却只做很少计算，就更容易被 HBM 带宽限制。

数据从哪来、怎么靠近计算位置——这个问题现在已经有了答案。下一个问题是：数据送到附近之后，哪些硬件单元真正消耗它。

## 3. SM 架构

### 3.1 SM 是 GPU 的基本计算单元

可以先把 GPU 想成由很多计算”车间”组成，每个主要车间就是一个 SM（Streaming Multiprocessor）。不同 GPU 代际的 SM 细节会变化，但几个核心组件是稳定的：
...
不同精度路径对应不同的 Tensor Core 通道，决定了矩阵乘能走到哪条硬件加速路径：
...
| INT8 / INT4 | 推理量化常见，量化策略属于后续专题；

精度路径决定了 Tensor Core 能否介入、以及峰值吞吐的天花板在哪里。搞清楚这一点之后，再看 Tensor Core 在一次矩阵乘里具体怎么工作。
...
但到这里，我们讲的仍然是硬件视角：数据放在哪里，SM 里有哪些执行单元，一个 tile 如何被计算。真实写程序时，开发者并不会直接对某个 SM 下命令，也不会手动指定某个 Warp 去执行哪一段输出。还需要一个软件层面的执行模型，把”我要做一次矩阵乘”这样的请求组织成 GPU 能调度的任务。

## 4. CUDA 执行模型

### 4.1 从软件层次映射到硬件

框架 API 最终会落到怎样的执行模型上？以 NVIDIA GPU 为代表，CUDA 就是连接”软件请求”和”硬件执行”的核心抽象：上层框架提交一段 GPU 上执行的任务，CUDA 把它组织成一批可以被 GPU 调度的并行工作单元。
...
Occupancy 的意义在于延迟隐藏。访问 HBM 的延迟很高，一次读取往往需要数百个时钟周期。如果一个 SM 上只有很少 Warp，一旦它们都在等数据，计算单元就会空闲。更高的 occupancy 意味着有更多就绪 Warp 可供调度器切换——当一个 Warp 在等待内存数据时，调度器可以立刻切到另一个已就绪的 Warp 继续执行，从而把访存等待时间”藏”进有效计算里。

但”occupancy 越高越好”也是错误的。它和性能的关系取决于算子类型。

| 算子类型 | Occupancy 的重要性 | 更关键的观察 |
| --- | --- | --- |
| 数据搬运压力大的算子 | 通常更敏感 | LayerNorm、Softmax、element-wise、decode KV cache 读取等场景，需要更多并发来隐藏访存延迟；
| 计算密集型算子 | 不一定越高越好 | 大 GEMM 更关键的是 Tensor Core 利用率、tile 形状、数据复用、流水是否充分；
...
3.4 节描述的是单个输出 tile 在硬件上如何执行。但 $Y=XW$ 的输出矩阵有 1024 个这样的 tile，它们不会自动分配到 GPU 上——需要软件把这 1024 个工作单元编号、打包、提交，再让硬件调度器决定谁先跑、跑在哪个 SM 上。这就是 Grid/Block/Thread 层级真正要解决的问题。
