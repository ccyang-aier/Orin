...
> [!Quote] 本篇导读
> 学习大模型推理时，很多性能现象表面上像框架参数问题：同一个模型，batch size 翻倍但吞吐没有线性翻倍；prefill 一次处理长上下文看起来很快，decode 逐 token 生成却容易拖慢；换成峰值更高的 GPU，真实延迟仍可能不如预期。
>
> 这些问题不能只靠“GPU 很快”解释。模型推理最终会落到一批张量计算：token 被表示成向量，权重参与矩阵乘，KV cache 在显存中反复读写，kernel 被提交到 GPU 上排队执行。性能好不好，取决于工作能否被并行拆开、数据能否及时送到计算单元、硬件单元能否持续忙碌，以及多卡场景中通信是否挡在关键路径上。
>
> 所以本篇不急着讨论具体优化技巧，而是先建立 GPU 的底层心智模型。我们会从 CPU 与 GPU 的设计差异开始，用 `Y=XW` 这个贯穿例子，把存储层级、SM/Warp/Tensor Core、CUDA 执行模型和 compute-bound / memory-bound / communication-bound 三类瓶颈串起来。读完后，再看后续的 FlashAttention、KV cache、TP/PP/EP 或 serving scheduler，就会更容易判断：某个机制到底在解决哪一类性能问题。
...
上一小节说 GPU 喜欢“海量相似任务”，但这句话还比较抽象。下面引入一个具体例子：大模型里常见的 `Linear` 层。先不用急着看公式，可以把它理解为“把一批输入数字乘上一块权重，得到一批新的数字”。
...
### 2.2 同一块权重在 prefill 和 decode 中为什么压力不同
...
上一节讲的是数据怎么靠近计算单元。现在看真正消费这些数据的硬件。可以先把 GPU 想成由很多计算“车间”组成，每个主要车间就是一个 SM（Streaming Multiprocessor）。不同 GPU 代际的 SM 细节会变化，初学者也不需要一开始记住所有硬件细节，先抓住几个稳定组件即可：
...
大多数大模型学习者不需要一开始就手写 CUDA，但必须知道框架 API 最终会落到怎样的执行模型上。到这里我们已经知道硬件如何组织计算，但 PyTorch 代码不会直接操作 SM 和 Warp，所以还需要一层从软件任务到硬件执行的映射。
...
现在回到导读中的那些现象：为什么 batch size 翻倍后吞吐不一定翻倍，为什么 prefill 和 decode 的速度差异很大，为什么换了峰值更高的 GPU 也不一定线性变快。前面的几件事已经连起来了：工作要能拆开，数据要能送到附近，计算单元要被喂饱，软件还要把任务排进 GPU。带着这些背景再看性能瓶颈，compute-bound、memory-bound、communication-bound 就不再只是三个英文标签。
