...
---
# Pipeline Parallelism（PP）是什么

> [!Quote] 本篇导读
> PP（Pipeline Parallelism，流水线并行）的第一直觉是 **把模型按层切到多张 GPU 上**，但真正难懂的部分不只是层怎么切，而是切完以后怎样让这些 stage 少空等。TP 的核心在层内张量和 collective communication，DP 的核心在副本、分流和状态边界，PP 的核心则落在 stage、activation handoff、micro-batch、pipeline bubble、stage balance，以及吞吐和延迟之间的折中。
...
当模型有很多 Transformer layers，单张 GPU 放不下完整权重，TP 已经把单层内部的矩阵切开，但继续扩大 TP size 又会让层内 collective 频繁跨节点，通信开销变重，DP 也帮不上忙，因为 DP 的前提是已经存在一个可独立运行的模型执行单元。
...
输入 tokens 先经过 Stage 0 得到 hidden states，这些 hidden states 被发送给 Stage 1，Stage 1 继续计算后再把新的 hidden states 发送给 Stage 2，直到最后一个 stage 输出 logits。

![PP 按层切分|900](imgs/pp-stage-partition-cn.png)

这张图要抓住三个点。

第一，PP stage 持有的是不同的层，Stage 0 和 Stage 1 都不是完整模型，单个 stage 通常不能对外提供完整推理服务。
...
PP 很容易被误读成多张 GPU 各做一段，所以也算分流。这个说法只看到了模型被拆开，却忽略了执行链路仍然是一整条 pipeline。
...
按层切分以后，一个很自然的问题会出现：既然一个请求要依次走过所有 stage，PP 会不会让单请求更快？通常不能这样期待。
...
PP 的完整理解要同时站在两个维度上：空间上按 layers 放置权重，时间上用连续工作填满流水线。
...
把 PP 和 TP、DP 放在一起看，关键差异不在 GPU 数量，而在切分对象和一次请求的执行路径：
...
把这些词放稳以后，PP 的核心就不再是用了几张 GPU，而是**每个 stage 持有什么、什么时候工作、什么时候等待**。
...
这里的 utilization 是平均 stage 利用率和吞吐直觉，不是只有最后一个 stage 的输出才算有效工作。对应的 bubble 占比约为：
...
PP 最早被系统化讨论时，训练场景占据了很大篇幅。推理场景仍然需要认识这些调度名字，因为很多框架和文档会沿用它们。
...
有些层可能包含更重的 MLP，有些 stage 可能因为 embedding、LM head、MoE expert、KV cache 布局或硬件位置而更慢，跨节点 stage 之间还会叠加网络传输延迟。
...
vLLM 文档给出的部署建议也来自这里：如果单节点内可以用 TP 放下模型，优先在节点内使用 TP；当跨节点 TP 代价太高，或模型无法在单节点内放下时，再考虑 PP 或 TP+PP 组合。
...
这张图的关键边界是：DP replica 之间不传 activation。Router 把不同请求分给不同完整 pipeline group，进入某个 pipeline group 后，请求才沿着 Stage 0、Stage 1、Stage 2 依次前进。TP 通信则发生在每个 stage 内部的 TP group 中。
...
| Hugging Face Transformers / Accelerate | Transformers 常见 `tp_plan="auto"`；Accelerate 有 `prepare_pippy` | TP 推理更常见，PP 多依托 PyTorch/PiPPy 路线 | `tp_plan` 属于 TP 路线；`device_map` 把层放到多设备不一定等于有 schedule 的 PP |
...
如果跨机链路很弱，而 stage 间 activation 很大，也要谨慎。PP 能避免跨节点 TP collective，但 activation send 仍然可能成为新瓶颈。
...
PP 的第一层判断是切分对象。它切的是 Transformer layers，每个 stage 持有连续层段，stage 之间传 hidden states / activations；它不切请求，也不切同一层内部的矩阵分片。

第二层判断是执行单元。单个 stage 不是完整模型副本，通常不能独立对外返回一次推理结果；完整 pipeline group 才是可接请求、可被 DP 复制的执行单元。把 stage 当成 replica，会把 PP 和 DP 的边界混掉。

第三层判断是时间调度。PP 不会让单个 token 绕过层序依赖，也不会天然降低单请求延迟；它依靠 micro-batches、chunked prefill、批量请求或多条 in-flight sequences 让不同 stage 同时处理不同工作。没有足够连续工作时，pipeline bubble 会迅速吞掉并行收益。

第四层判断是工程代价。PP 能分摊权重显存，部分分摊按层产生的 KV cache 压力，并减少某些跨节点 TP collective；同时它会引入 stage 间 activation send、stage balance 难题、调度复杂度和可能的 latency penalty。PP size 越大，stage 更细，但 bubble、边界和通信路径也会增加。

第五层判断是组合拓扑。真实部署里，PP 常和 TP、DP 一起出现：TP 在 stage 内部切 layer 计算，PP 在连续 stage 之间传 activation，DP 在完整 pipeline group 之间复制执行单元。看到 `pipeline_parallel_size` 时，比 GPU 总数更重要的问题是：
...
12. [Hugging Face Transformers: Distributed inference with tensor parallelism](https://huggingface.co/docs/transformers/main/perf_infer_gpu_multi)：用于反向区分 `tp_plan="auto"` 属于 TP 路线，而不是 PP 主入口；
...
19. 单选：在当前精度、上下文长度和 KV cache 预算下，一个 70B 模型单节点 8 卡仍放不下；继续扩大 TP 会跨节点做高频 collective，业务有较高并发请求。更合理的优先方向是？
...
21. A、C、D。推理中每个 stage 主要维护自己负责 layers 的 KV cache；stage 间 forward 主要传 activations。PP 能分摊权重和部分 KV cache 压力，但不会让 KV cache 消失，长上下文和高并发仍会累积运行时状态。若还叠加 TP，KV cache 和 activation 的实际分布会继续受 TP ranks、KV heads 和框架实现影响，但 PP 的核心直觉仍是按 layers/stages 分布；
