...
很多人第一次遇到 CP，不是在训练论文里，而是在一个看起来已经足够分布式的推理服务里。这个服务可能已经用 TP 处理大模型，用 DP 或外部负载均衡承接高请求量，用 PP 分摊较深的模型层数；如果是 MoE 模型，expert 也可能已经通过 EP 分布到多张 GPU 上。

这些并行策略解决的是不同维度的问题。TP 让同一层的矩阵计算能够跨卡完成，DP 让多个副本处理不同请求，PP 让不同层段分布到不同设备，EP 让专家权重和 expert routing 不再挤在同一张卡上。可是当用户发来一个 128K、256K，甚至更长的 prompt 时，系统仍然可能出现几类很具体的症状：
...
CP 面对的就是这类单请求压力：请求数量未必多，但一条请求的上下文本身太长，Attention、activation 或 KV cache 已经成为系统瓶颈。

![长上下文压力|900](imgs/cp-long-context-pressure-cn.png)

这张图的重点不在某一个参数，而在长上下文给 prefill 和 decode 带来的不同压力。prefill 的输入 token 很多，Attention 计算和激活显存会跟着变重，TTFT 容易被拉长；decode 每步只生成很少的新 token，却要持续访问历史 KV cache，长上下文和高并发会让 KV cache 变成显存和带宽热点。

### 1.2 CP 的定义与边界

把前几篇的并行策略放在一起，CP 的位置会更清楚：
...
这一定义有两层边界。第一，CP 切的是同一条序列，不是把不同请求分给不同 GPU，后者更接近 DP；第二，CP 的复杂协作主要发生在 Attention 和 KV cache 边界，不是让所有算子都变成全局通信，很多逐 token 模块在本地 chunk 上就能运行。

更正式地说，**Context Parallelism 指的是沿 sequence length 或 context length 维度切分模型输入、激活、Attention 中间状态或 KV cache，让一个上下文中的不同 token chunk 分布在多个设备上计算与存储，并在 Attention 或 decode 读取 KV 的边界通过通信恢复数学等价的全局上下文视图。**
...
TP 典型会在 $H$、head 或 intermediate 维度上切，DP 典型会在 $B$ 或请求集合上切，CP 则切 $T$：
...
![CP 上下文切分示意|900](imgs/cp-context-sharding-mental-model-cn.png)
...
因此，CP 不是通用的多卡加速按钮。实际判断时要先排除几个不属于 CP 主线的问题：
...
CP 的价值在于，当一条上下文本身太长，导致 Attention、activation 或 KV cache 成为瓶颈时，给系统增加一个沿 context 维切分的能力。

## 2. sequence 维的切分方式

CP 的定义落到执行路径上，首先要把两个动作分清：DP 改变请求集合的路由，CP 改变单条请求内部 token 的设备布局。假设有 4 条请求，每条 8K tokens，DP 会把这些请求分给 4 个 replica：
...
沿 sequence 维切开以后，并不是 Transformer 里的所有模块都需要跨 token 交互。LayerNorm 或 RMSNorm 通常在 hidden 维度上对每个 token 独立归一化，MLP 也是对每个 token 的 hidden vector 做同一组线性变换与非线性变换。只要每个 rank 拿到自己负责的 token chunk，就能在本地完成这些计算。
...
这里 $t$ 是 token 位置。$Y_t$ 只依赖 $X_t$，不依赖 $X_0,\ldots,X_{T-1}$ 的其他位置。沿 $T$ 切开后，每个 rank 仍然能独立计算自己的 token chunk。这也是 NVIDIA Megatron Core 文档强调的关键点：CP 会沿 sequence 维切网络输入和所有 activation，但除 Attention 以外的模块通常可以照常运行，真正需要额外处理的是 Attention。
...
某个 GPU 持有 $Q_i,K_i,V_i$。它要得到自己负责的输出 $O_i$，并不是只做：

$$
\text{Attention}(Q_i,K_i,V_i)
$$

因为 $Q_i$ 里的 token 还需要 attend 到同一序列中按 mask 可见的其他 chunk。数学上更准确的写法是：

$$
O_i = \text{Attention}(Q_i,[K_0,K_1,K_2,K_3],[V_0,V_1,V_2,V_3];\text{mask}_i)
$$

这里的 $\text{mask}_i$ 很重要。对于 decoder-only causal attention，靠前 token 不能看到未来 token；通信上可以让完整 K/V chunk 被收集或流过本地 rank，但真正参与 softmax 的位置仍然由 causal mask 决定。换句话说，CP 要恢复的是**按注意力规则可见的全局上下文视图**，不是让所有 token 无条件看见未来。

其他 rank 也一样。于是 CP 的核心通信问题可以压缩成一句话：**每个 rank 的 Q 是本地的，但它需要在 Attention 边界获得按 mask 可见的全局 K/V 视图。**
...
实用判断方法是看它有没有处理 Attention 或 KV cache 的跨 rank 交互。如果一个方案只让 LayerNorm、Dropout 等模块在 sequence 维上分片，但 Attention 仍然没有解决全局 K/V 问题，它更接近狭义 SP；如果它把长上下文 Attention 或 KV cache 的跨 rank 交互纳入设计，它才接近这里讨论的 CP。
...
所以 CP 不是 CP size 越大越好，而是当 sequence/KV 压力足够大，且通信能被计算或内存收益抵消时才有价值。

## 3. CP Attention

第 2 章已经把 CP 的核心边界收束到 Attention：本地 token chunk 可以独立跑很多逐 token 模块，但本地 Q chunk 必须在 Attention 边界获得全局 K/V 视图。不同 CP 实现的差异，主要就体现在这个全局视图怎样被恢复：一次性收集、沿环传递，或者通过 all-to-all 改变数据布局。
...
$$
O_i=\text{Attention}(Q_i,K_{\text{full}},V_{\text{full}};\text{mask}_i)
$$

这种方式最容易理解，也很适合中等长 prompt：每个 rank 只需要输出自己那段 query 的结果，但 Attention 计算中能看到完整上下文，再由 mask 决定每个 query token 实际可见的位置。

它的代价也很清楚：每个 rank 在 Attention 时临时持有完整 K/V。如果上下文继续变长，完整 K/V 本身也放不下，就要换成逐块流动的方式。

### 3.2 Ring Attention

Ring Attention 的关键是不一次性聚齐所有 K/V，而是让 K/V chunk 沿环传递。每个 rank 的 Q chunk 不动，K/V chunk 一轮一轮传过来。每收到一段 K/V，就用 blockwise/online softmax 更新本地 attention state，包括 running max、normalizer 和输出状态，而不是把每个 block 独立 softmax 后的结果直接相加。

![Ring Attention 中的 pass-KV 流程|900](imgs/cp-ring-attention-pass-kv-handdrawn-cn-v3.png)

在 4 张 GPU 的例子里，GPU 0 起初有 $Q_0,K_0,V_0$。第一步它用 $Q_0$ 与本地 $K_0,V_0$ 更新 attention state；后续步骤收到相邻 rank 传来的远端 K/V block，先依据 mask 判断哪些位置对 $Q_0$ 可见，再用同一套 online softmax 状态合并规则更新。经过若干步后，$Q_0$ 已经按 mask 规则访问过所需 K/V block，得到的 $O_0$ 与完整 Attention 在数学上等价。

这里有两个关键点。

第一，Ring Attention 通常不是稀疏注意力，也不是把远处 token 丢掉。它的目标是 exact attention，只是把一次大矩阵注意力拆成多段 blockwise 计算，并通过 online softmax 的归一化状态合并保持等价。
...
2. 到 Attention 时，通过 all-to-all 把数据布局从每个 rank 有一段 sequence，变成每个 rank 有完整 sequence 的一部分 heads；
...
一些系统会使用更细的 token 排布、load balancing、interleaving 或特殊 mask 处理，让不同 rank 的 Attention 负载更均衡。Megatron Core 文档也提到 CP 需要避免 lower-triangle causal masking 带来的额外工作，同时保持负载均衡。这里的核心不是画出 mask，而是理解一件事：**sequence 切分改变了每个 rank 面对的三角 Attention 工作量，系统需要显式处理这个不均衡。**
...
可以用一个极简例子建立直觉：如果 $T_{\text{local}}$ 不变，但模型从 MHA 变成 GQA，K/V heads 变少，$H_{KV}$ 就会下降；同样的 pass-KV 或 all-gather 路径上传输的 K/V 数据量也会下降。反过来，如果只是把 CP size 调大但跨节点链路很慢，$T_{\text{local}}$ 下降带来的显存收益可能会被更多通信轮次抵消。
...
前面讨论的是 CP 在 Attention 边界如何恢复全局上下文视图。进入推理服务后，同一个 CP 概念还要按生命周期拆开：prefill 关心长 prompt 一次性进入模型时的计算与 TTFT，decode 关心每一步生成时如何读取已经增长起来的 KV cache。

### 4.1 Prefill、decode 与 DCP

推理里的 CP 必须把 prefill 和 decode 分开讲，因为它们的工作形态完全不同。prefill 阶段处理用户输入的 prompt，假设 prompt 长度是 $T$，模型需要为这些新 token 计算 Q/K/V，并完成每一层的 Attention 与 MLP，长 prompt 的 prefill 往往决定 TTFT。decode 阶段每步通常只新增一个或少量 token，计算新 token 的 Q/K/V 并不大，但它要读历史 KV cache，上下文越长、并发越高，KV cache 越容易成为显存和带宽瓶颈。

vLLM 的 Context Parallel Deployment 文档也明确把二者拆开：长上下文 prefill 关注 TTFT，长上下文 decode 关注为 KV cache 腾出空间以提高 batch size 和吞吐。沿着这条生命周期看，prefill CP 的基本思路是把一个长请求的 $T$ 个新 token 分成 $N$ 个 chunk，每个 GPU 计算其中一段 Q/K/V。
...
需要注意的是，截至 2026-05-27 核对，vLLM Context Parallel Deployment 页面标注日期为 2026-01-29，这两类 prefill CP 方案仍处于 active development。理解机制时可以先掌握这两类方案，但部署时要以当前框架版本、backend 支持和 release note 为准。

decode CP 的核心不是把一个 token 的计算切得更碎，而是处理 KV cache 的分片。
...
更具体地说，在 vLLM 文档描述的 DCP 语境中，`dcp_size` 通常受 `tp_size` 与每个 TP group 内 KV heads 数量共同约束，常见可用范围可以理解为 $[1, \text{tp\_size}/H_{KV}]$ 这一类上界；如果 DCP size 继续变大，KV 重复会下降，但 decode 读取分布式 KV 的通信会增加。vLLM 还会用 token 维 interleaving 的方式让 decode 增长中的 KV cache 分布到不同 DCP rank，因此支持情况需要同时看模型结构、KV heads、attention backend 和当前版本文档。

### 4.2 工程边界与状态布局

DCP 的收益来自减少 KV cache duplication，但它把 decode 的状态读写变成了更细的分布式布局，因此需要同时判断模型结构、链路质量、backend 支持和运行时状态管理。更适合考虑 DCP 的场景通常具备这些条件：
...
vLLM 文档给出的实践建议也很朴素：先增加 TP size 直到性能满意，再加 DCP 来减少 KV cache duplication。这个顺序背后的原因是 decode 阶段 query token 有限，过度切 context 可能让非 Attention 层上的 GPU 工作不充分，而且 DCP 的收益必须扣除额外通信成本。

长上下文推理经常伴随 prefix cache、automatic prefix caching、会话续写、检索增强上下文复用等机制。CP 不会取消这些状态边界，只会让状态布局更复杂。进入生产系统时，需要继续追问：
...
CP 解决的是长上下文和 KV cache 维度的问题，但真实推理系统很少只启用一种并行策略。模型权重、层数、请求量、expert 分布和上下文长度可能同时成为约束，因此要把 CP 放回完整并行地图里，先定位瓶颈，再决定它应该和 TP、DP、PP、EP 怎样组合。

### 5.1 先定位瓶颈

把 CP 放进整个并行策略地图里，最重要的问题仍然是：瓶颈在哪里？

![CP 与其他并行策略的选型地图|900](imgs/cp-parallelism-decision-map-handdrawn-cn-v2.png)
...
这是 Megatron Core 训练 CP 语境中对 CP 与 TP、PP、DP 组合关系的基本表达。需要注意，不同框架的参数名、group 组织方式和支持矩阵会不同，不能只拿这个乘法式替代具体部署检查；它也不能直接套到 vLLM DCP 的 worker 数计算上，因为 DCP 复用已有 TP group。
...
在 Megatron Core 的示意里，TP 和 CP 的通信点不同：Attention 附近的通信更多属于 CP，其余一些层内通信更多属于 TP。初学时不要把所有 collective 都归为同一种多卡通信，要问清楚它是在恢复 hidden/head 维，还是在恢复 context/KV 维。
...
在 MoE 模型中，长上下文会增加 token 数，token 数增加会进一步影响 expert routing 的负载形态。此时系统里可能同时有三类路由：
...
| 系统或框架 | 常见入口 | 执行直觉 |
| --- | --- | --- |
| Megatron Core nightly | `context_parallel_size` | 训练路径中沿 sequence 维切输入和 activation，并在 Attention 交换 K/V |
| PyTorch experimental CP（unstable / 2.12.0+cu130 教程） | `context_parallel()`、`buffer_seq_dims`、`set_rotate_method()` | 在上下文管理器中切分 buffer，并把 SDPA 替换为 CP 版 Ring Attention |
| DeepSpeed Ulysses | `--ds-sequence-parallel-size`、`DistributedAttention` | sequence shard 与 attention head 布局通过 all-to-all 转换 |
| vLLM prefill CP（页面日期 2026-01-29） | prefill context parallel 相关能力 | 长 prompt 按 token chunk 分摊 prefill 计算 |
| vLLM decode CP / DCP（页面日期 2026-01-29） | `-dcp <size>` | 在 TP group 内沿 context tokens 维减少 KV cache duplication |
...
CP 的位置可以从一个分布式推理系统的压力来源里看清楚。TP 处理 hidden/head/weight 维度，PP 处理层段，DP 处理不同请求，EP 处理 MoE experts；当这些维度已经被拆过，一条超长上下文请求仍然会让 prefill、Attention 和 KV cache 变重时，系统才需要沿 context/sequence 维继续切分。

切开 sequence 以后，普通逐 token 模块通常仍然可以在本地 chunk 上运行，真正决定 CP 难度的是 Attention。每个 rank 的 Q chunk 是本地的，但它必须读到按 mask 可见的全局 K/V，最终输出仍然保持在自己负责的 token chunk 上。All-gather KV、Ring Attention 和 Ulysses all-to-all 都是在回答同一个底层问题：怎样让本地 token chunk 在不丢失数学语义的前提下获得足够的全局上下文。

推理侧还要把 CP 拆成 prefill 和 decode 两条线。prefill CP 面向长 prompt 的一次性计算和 TTFT，decode CP 或 DCP 面向持续增长的 KV cache、batch size 和吞吐上限。vLLM 的 DCP 复用已有 TP group，并不增加 GPU 数，它减少的是 KV cache duplication，换来的是每步 decode 读取分布式 KV 的通信成本。

因此，CP 的工程判断不能只看参数是否存在。更稳妥的顺序是：先确认 workload 里是否真的有长上下文压力，再区分瓶颈来自 prefill TTFT 还是 decode KV cache；随后检查模型的 KV heads、attention backend、RoPE/mask 支持、prefix cache 布局、TP/CP/DP/PP/EP group 排布和通信拓扑；最后用真实 prompt 长度、输出长度、并发与 cache 命中率 benchmark 端到端 SLO。

CP 最有价值的区域，是**长上下文压力足够大**与**通信成本可控**的交集。看到 CP、PCP、DCP 或类似配置时，最关键的问题不是用了几张卡，而是这些问题是否有清楚答案：切的是哪段 context，Attention 如何读全局 K/V，KV cache 如何分布，通信在哪条链路上，prefill 和 decode 的 SLO 是否分别变好。
...
6. 单选：Ring Attention 的核心思想是什么？
    ...
    B. 让 K/V chunk 沿环传递，本地 Q 逐块和收到的 K/V 更新 online softmax attention state；
...
16. 单选：下面哪一句最接近本章对 CP 的总结？
    A. CP 就是把所有 GPU 显存合成一个透明大池；
    B. CP 就是让更多请求进入同一个副本；
    C. CP 是在长上下文压力下切 context，让本地 token chunk 通过 Attention 边界读到按 mask 可见的全局 K/V；
    D. CP 只用于 MoE expert 路由；

17. 多选：decode 阶段的 KV cache 设计需要关注哪些状态边界？
    A. prefix cache 命中与 KV 布局是否一致；
...
19. 单选：一个模型使用 MLA 后只有 1 个 KV head，当前部署为 `TP=8`，decode 阶段 KV cache 显存成为主要瓶颈。若 attention backend 支持 DCP，最合理的判断是？
    A. 可以考虑 `DCP=8` 来减少 KV cache duplication，但要评估 decode 通信开销；
    B. 必须把 DP 从 1 增加到 8，因为 DCP 会增加 GPU 数；
    C. 应优先使用 CP 替代所有 TP，因为 TP 与 DCP 不能共存；
    D. 应关闭 prefix cache，因为 DCP 与 KV cache 无关；

20. 多选：下面哪些场景更适合 `partial query, partial K/V` 的 prefill CP 思路，而不是 `partial query, full K/V`？
    A. prompt 极长，单卡临时持有完整 K/V 已经不可接受；
    B. 希望通过 Ring Attention 等方式逐块交换 K/V；
    C. prompt 只是中等长度，完整 K/V 可以临时 all-gather，目标主要是降低 TTFT；
    D. attention backend 和通信调度能够支持分块或流式执行；

21. 多选：一个 16 GPU 集群同时有大量 2K 短请求和少量 128K 长请求。哪些部署判断更合理？
    A. 可以考虑分层路由：短请求走高 DP、低 CP 的副本，长请求走低 DP、高 CP 的副本；
    B. 所有请求都强制进入同一个最大 CP group，通常一定能提升整体吞吐；
    C. 需要分别 benchmark 短请求吞吐、长请求 TTFT、decode TPOT、KV cache 占用和通信耗时；
    D. CP 只应在能缓解长上下文瓶颈且通信成本可控时启用；

### 9.2 答案与题解

错题回看建议：1-5 题回看第 1、2 章；6-8、20 题回看第 3、4 章；9-11、17、19 题回看第 4 章；12-16、18、21 题回看第 5、6、7 章。
...
6. B。Ring Attention 保持本地 Q chunk，逐块接收 K/V chunk，并用 online softmax 状态合并得到与完整 Attention 等价的结果；

7. A、B、D。Ring Attention 通常追求 exact attention，并通过 blockwise 计算、online softmax 状态合并与通信重叠降低内存压力；C 错在忽略了 mask 与负载均衡问题；
...
16. C。这句话同时抓住了 CP 的切分维度、适用压力和 Attention 的全局 K/V 边界；加上 mask 是为了避免把 causal attention 误解成可以看见未来 token；

17. A、B、C。decode 中 KV cache 是持续增长的运行时状态，prefix cache 命中是否能对齐 CP/DCP 后的 KV 布局、rank 布局和新 token KV 的追加方式都会影响正确性与性能；D 与 CP 无关；

18. B。并行参数只是入口，真正要判断的是 group 组织、瓶颈匹配和通信拓扑。A 计算错误，C 和 D 都混淆了不同并行策略的边界；

19. A。MLA/GQA/MQA 场景下 KV heads 较少，继续增大 TP 可能造成 KV cache 重复；vLLM 文档也用 MLA enabled 且 KV head 很少的 case study 解释 DCP 为什么有价值。DCP 可以在已有 TP group 内沿 context tokens 维减少 duplication，但会增加 decode 通信。B 错在把 DCP 理解成增加 GPU 数；C 错在混淆 TP 与 DCP；D 错在否认 DCP 与 KV cache 的关系；

20. A、B、D。`partial query, partial K/V` 面向完整 K/V 也难以临时放下的极长 prompt，通常需要 Ring Attention 等分块交换机制和 backend 支持。C 更接近 `partial query, full K/V`；

21. A、C、D。混合 workload 下，短请求和长请求的最优并行形态可能不同，应通过分层路由和真实 benchmark 判断；分层路由不是固定答案，资源比例仍要由 workload benchmark 决定。B 过于绝对，最大 CP group 可能让短请求承担不必要通信成本并降低整体吞吐；
