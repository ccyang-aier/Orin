...
但是当用户发来一个 128K、256K，甚至更长的 prompt 时，服务仍然会出现几类很具体的症状：
...
【批注，加一个过渡】
【批注，审视所有标题，不要把一个未提交铺垫或引入的结论或问题作为标题，很莫名其妙】
【批注，合并2节所有子小节】
...
那么某个 GPU 持有 $Q_i,K_i,V_i$。它要得到自己负责的输出 $O_i$，并不是只做：
...
**每个 rank 的 Q 是本地的，但它需要在 Attention 边界获得按 mask 可见的全局 K/V 视图。**
...
Ring Attention 可以理解成“不要一次性聚齐所有 K/V，而是让 K/V chunk 沿环传递”。每个 rank 的 Q chunk 不动，K/V chunk 一轮一轮传过来。每收到一段 K/V，就用 blockwise/online softmax 更新本地 attention state，包括 running max、normalizer 和输出状态，而不是把每个 block 独立 softmax 后的结果直接相加。
...
【批注，7、8小节合并，并把标题改为本章小结，重构内容，不要说常见误区和心智模型，对本章做一个阶段性小结，条理清晰循序渐进，内容和见解有深度】
...
错题回看建议：1-5 题回看第 1、2 章；6-8、20 题回看第 3、4 章；9-11、17、19 题回看第 4 章；12-16、18、21 题回看第 5、6、8 章。
