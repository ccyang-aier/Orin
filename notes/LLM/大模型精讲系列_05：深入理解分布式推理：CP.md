...
19. A。MLA/GQA/MQA 场景下 KV heads 较少，继续增大 TP 可能造成 KV cache 重复；DCP 可以在已有 TP group 内沿 context tokens 维减少 duplication，但会增加 decode 通信。B 错在把 DCP 理解成增加 GPU 数；C 错在混淆 TP 与 DCP；D 错在否认 DCP 与 KV cache 的关系；
...
21. A、C、D。混合 workload 下，短请求和长请求的最优并行形态可能不同，应通过分层路由和真实 benchmark 判断。B 过于绝对，最大 CP group 可能让短请求承担不必要通信成本并降低整体吞吐；
