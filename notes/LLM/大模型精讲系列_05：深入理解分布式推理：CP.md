...
在 4 张 GPU 的例子里，GPU 0 起初有 $Q_0,K_0,V_0$。第一步它用 $Q_0$ 与本地 $K_0,V_0$ 更新 attention state；第二步收到某个相邻 rank 传来的 $K_3,V_3$，继续用同一套 online softmax 状态合并规则更新；经过若干步后，$Q_0$ 已经按 mask 规则访问过所需 K/V block，得到的 $O_0$ 与完整 Attention 在数学上等价。
