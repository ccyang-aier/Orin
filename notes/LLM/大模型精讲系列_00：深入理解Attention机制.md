...
阅读路径：本文是 [[大模型精讲系列_00：Transformer架构及原理|00-B Transformer 架构及原理]] 的机制前置篇；如果已经熟悉 Q/K/V、mask 与 Multi-Head Attention，可以直接进入 Transformer 的整体架构。
...
下一篇阅读：[[大模型精讲系列_00：Transformer架构及原理|00-B Transformer 架构及原理]] 会把 Attention 放进完整网络骨架里，继续解释位置编码、Encoder/Decoder、FFN、残差归一化与训练/推理数据流。
