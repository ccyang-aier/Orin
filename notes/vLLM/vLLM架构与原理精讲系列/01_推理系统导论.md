...
大模型推理最粗略的看法，是一次函数调用：用户输入 Prompt，模型执行，系统返回回答。这个说法有用，因为它先把问题压到一条请求链路上，但它也很容易遮住真正的工程复杂性。

![](imgs/01_inference_request_flow.png)

如图，哪怕只看一次完整的推理请求生命周期，系统内部也已经包含多个连续环节：

1. **用户发起推理请求**，模型接收输入 Prompt；
2. **Tokenization（分词）**：Prompt 被切分成一个个 Token，转化为模型能理解的数字序列；
3. **Prefill（预填充）**：模型一次性并行处理所有输入 Token，计算并缓存它们的 KV（Key-Value）状态，为后续生成建立上下文；
4. **Decode（逐步解码）**：模型进入自回归生成阶段，每次生成一个新 Token，再把新 Token 作为下一步输入继续推进，直到遇到终止符或达到长度上限；
5. **Detokenization（反分词）& 返回结果**：生成的 Token 序列被还原成人类可读的文字，流式或一次性地返回给用户。

这条链路看起来仍然简单，但落到生产系统里会迅速变复杂：请求可能排队等待资源，输入 Prompt 需要被整段并行处理，第一个 Token 可见之后还要一步步自回归生成，整个过程中还要不断保存、读取和复用中间状态。

当并发请求逐渐增多，这条链路就不再只属于某一个用户，它会演变成一个多请求争抢共享资源的调度问题，而如何在延迟、吞吐与资源利用率之间找到平衡，正是大模型推理系统真正的复杂性所在。

## 1. 为什么需要理解推理系统

推理系统的核心工作，是在固定模型权重的前提下，把用户请求稳定、高效、可观测地转化成输出 token，这里有三个关键条件：

- **固定权重**：推理阶段通常不更新模型参数，模型更像一个只读计算图；
- **动态请求**：每个请求的输入长度、输出长度、到达时间、取消时间都可能不同；
- **系统目标**：推理服务要同时关注交互体验、吞吐、显存、成本、稳定性和公平性；

如果只有一个用户、一个 prompt、一个输出，推理看起来很简单，但生产环境可能会把问题变成下面这样：
...
此时，系统不能只考虑模型怎么前向计算，它还要分析先处理谁，怎么合批，缓存放哪里，怎么取缓存，什么时候暂停，如何避免一个长请求拖慢所有短请求，以及怎样用指标判断瓶颈在哪里。

所以，本章不从某个框架 API 开始，而是先盯住一条生成请求链路，顺着它逐步看见背后的状态、指标和资源约束。只有这张地图清楚了，后面学习 vLLM、TGI、TensorRT-LLM 或其他推理引擎时，才知道每个机制究竟在解决哪一类系统问题。
...
训练阶段的目标是**让模型学会参数**，系统会准备大量样本，计算 loss，反向传播，更新权重，训练任务通常可以把样本组织成相对规整的 batch，并围绕高吞吐的矩阵计算来优化。

推理阶段的目标是**响应请求**，模型权重固定不变，系统要把用户 prompt 转成一个个输出 token，这里的难点不只是计算，还包括请求到达的不确定性、输出长度的不确定性，以及每个请求在生成过程中留下的动态状态。
...
这个差异决定了：**推理优化不同于训练，训练更关心一次大规模计算如何跑满硬件，而推理更关心许多不规则请求如何高效共享硬件，同时保持用户可感知的响应速度。**

## 3. 一次生成请求的生命周期

一次典型的自回归 LLM 生成请求，可以拆成五个阶段。前两步把文本变成可执行请求，中间两步完成模型计算，最后一步把 token 流还原给用户：
...
其中，第三步和第四步是理解推理系统的主轴，它们对应两种非常不同的计算形态。

### 3.1 Prefill 与 Decode

在整个自回归过程中，LLM 推理可以先分成两个阶段理解：Prefill 负责读入完整 Prompt 并建立上下文，Decode 负责基于这个上下文逐 token 生成回答。它们不是简单的前后两段流程，而是两种硬件资源压力完全不同的计算形态。

**Prefill 阶段**处理用户输入的完整 Prompt，一次性完成所有 Prompt Token 的前向传播，并将每个 Token 在每一层、每个注意力头上对应的 Key 和 Value 写入 KV Cache。假设用户输入了 2,000 个 token，模型需要把这 2,000 个 token 作为上下文整体读入，计算每一层注意力和中间表示，再准备好后续生成要复用的状态。

Prefill 的特点是：

- 一次性处理大量 token，适合形成较大的矩阵计算；
- 计算量通常随 prompt 长度明显增长，长 prompt 会显著拉高请求启动成本；
- 更容易体现为 **Compute-bound**，瓶颈经常落在 GPU 算力利用上；
- 它通常是 TTFT 的重要组成部分，因为系统必须先建立上下文，才有条件返回第一个输出 token；

因此，Prefill 更像读完整份题目并建立上下文。

**Decode 阶段**每次只处理最新生成的一个 token，利用 KV Cache 中已经缓存的历史信息完成注意力计算，再采样出下一个 token。这个新 token 会被追加回上下文，成为下一步生成的输入，于是系统不断重复同一个动作，直到请求完成。

Decode 的特点是：

- 每一步只推进一个 token，天然具有自回归顺序依赖；
- 每一步都要读取模型权重和已有 KV Cache，虽然计算规模小，但访存压力很高；
- 输出越长，Decode 步数越多，用户等待完整回答的时间越容易被拉长；
- 更容易体现为 **Memory-bound**，瓶颈经常落在 GPU 显存带宽、KV Cache 读取和调度干扰上；
- 用户流式体验主要由 Decode 的节奏决定，也就是后续 token 能否稳定、顺滑地吐出来；

因此，Decode 更像 **一个 token 一个 token 地写答案**。第一个输出 token 决定系统什么时候开口，后续 token 的间隔决定系统说话是否顺滑。

![Prefill、Decode 与延迟指标](imgs/01_prefill_decode_timeline.png)

这张时间线说明了一个关键事实：用户感受到的响应不是单一数字。TTFT 衡量从请求进入测量边界到第一个输出 token 可见之间的等待，它会受到排队、前置处理、Prefill、首个输出 token 生成与采样等因素影响；TPOT 衡量第一个 token 之后，相邻输出 token 之间的平均间隔，它更接近用户看到模型持续输出时的节奏。

Prefill 和 Decode 的差异也会影响吞吐随并发变化的曲线。低并发时，Decode 往往无法充分利用 GPU 算力，因为每个请求每步只推进少量 token，却仍要搬运大量模型权重和状态；并发升高后，同一次权重读取可以服务更多请求，吞吐通常会先接近线性上升。再继续增加并发，系统可能逐渐触及算力、显存带宽、KV Cache 或调度上限，吞吐曲线会进入平台区。

![吞吐与并发的关系](imgs/01_throughput_vs_concurrency.png)

这条曲线不是固定规律，它取决于模型大小、Prompt 长度、输出长度、batch 组织和硬件配置。长 Prompt 会放大 Prefill 成本，少量并发就可能让算力先饱和；短 Prompt 的单次 Prefill 成本较低，通常需要更高并发才会达到饱和点。
...
有了请求生命周期、Prefill/Decode 和 KV Cache 这三层认知之后，性能指标就不再是一串孤立缩写。每个指标都在回答一个定位问题：用户等在哪里，系统忙在哪里，资源卡在哪里。
...
TPOT 更接近用户听到模型持续说话的节奏。TTFT 很低但 TPOT 很高，用户会感觉系统很快开口，但后面输出很慢；TTFT 较高但 TPOT 很低，用户会感觉系统先等了一下，然后回答很快刷完。
...
第一章到这里刻意停在抽象层。只要理解了这张全局地图，后续再进入具体推理引擎时，就不会把某个机制孤立地看成 **单个优化技巧**。更好的阅读方式是不断追问：这个模块在降低 TTFT、TPOT、显存压力、调度成本，还是在提升吞吐与稳定性。
