...
上一篇我们详细讲解了vLLM的调度器实现，我们知道了 Scheduler 不是简单的请求队列，而是每个 engine step 的吞吐指挥系统，它决定每一次迭代 running 请求继续推进多少 token，能否接纳新的 waiting 请求，KV Cache 是否还有 block，必要时谁要被抢占等等。

本章我们继续深挖调度器相关实现，并详细展开 vLLM 最近几个版本才新增支持的推理加速特性：**异步调度**。如果说普通 Scheduler 回答的是“本轮 batch 怎么组成”，那么 async scheduling 继续追问的是：当 GPU 正在执行本轮 batch 时，CPU 侧能不能提前准备下一轮 batch，从而减少 GPU 等 CPU 的空泡。
...
【批注，新增一段引入说明，重点说明 vllm 的异步调度实现的论文出处，做一个简单的引入介绍，比如：vLLM的异步调度源于对xx论文的实现，该论文链接是：xx，推荐大家也可以抽空阅读其原始论文，或许会有不一样的理解和收获】
