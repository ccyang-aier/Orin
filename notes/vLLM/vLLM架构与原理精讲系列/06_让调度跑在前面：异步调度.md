...
异步调度不是凭空出现的，它承接了三类问题：LLM serving 的 batch 边界应该多细、动态 KV 如何让 batch 足够灵活、CPU/GPU/通信资源能否从串行链条变成重叠流水。

Orca 不是人名，而是 OSDI 2022 的 Transformer 生成模型 serving 系统。它提出的关键设计是 iteration-level scheduling：请求不必等整个序列完成才释放 batch 位置，已经完成的请求可以离开，新到达的请求可以在下一次 decode iteration 加入，长短不一的输出不会被固定 request-level batch 卡死。对 vLLM 来说，这条线索贡献的是**每个 iteration 都可以重新组织 batch**的 serving 心智模型。

![iteration-level scheduling 的组批思想](imgs/06_iteration_level_scheduling.png)

PagedAttention 把 KV cache 管理变成 block/page 式的内存管理问题，让 continuous batching 可以更积极地接纳请求，而不会被连续显存分配和内部碎片拖垮。于是 vLLM 的吞吐优化从**如何组 batch**进一步变成**如何在动态请求、动态 KV、动态输出长度下持续保持 GPU 忙碌**。这条线索让 async scheduling 有了可操作的资源账本：提前调度下一步之前，Scheduler 必须知道 KV block、computed tokens 和请求状态是否还能自洽。

NanoFlow 也不是人名，而是一篇 LLM serving 系统论文。它的核心问题是：一次 LLM 生成并不只有 GPU compute，还包含 memory、network、CPU scheduling、input preparation 等异构操作，如果这些操作在单设备内严格串行，GPU、显存带宽、通信链路和 CPU 都会轮流等待。NanoFlow 的做法是把输入拆成更小的 nano-batch，并把 compute、memory、network 等操作错开执行，形成 operation-level pipeline。vLLM async scheduling 不等同于 NanoFlow，前者主要重叠 Scheduler/Worker 输入准备与 GPU forward，后者更细地重叠设备内部异构操作；二者共享的是**把资源等待变成可覆盖时间**的设计思想。
