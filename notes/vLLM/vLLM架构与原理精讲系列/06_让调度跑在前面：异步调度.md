...
这里的 `T_schedule` 指 Scheduler 扫描 running/waiting 请求、计算 token budget、申请 KV block、处理抢占和构造 `SchedulerOutput` 的时间；`T_input_prepare` 指 ModelRunner 把调度结果展开成 input ids、positions、block table、sampling metadata 等模型输入 tensor 的时间；`T_gpu_execute` 是 GPU 真正执行 forward、attention、MLP、sampling 相关计算的时间；`T_output_copy` 是 sampled token、logprobs、routed experts 等结果从 device 回到 CPU 可见状态的时间；`T_update` 则是 Scheduler 根据真实输出推进 request state、停止条件、KV cache 和统计信息的时间。
