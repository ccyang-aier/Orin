...
updated: 2026-06-10
description: 基于本地 vLLM V1 源码快照，拆解前端到 EngineCore、EngineCore 到 Worker 的控制面通信路径，说明 ZMQ、msgpack、collective_rpc、MessageQueue 与 output_rank 如何共同支撑推理控制流。
---
# 08 详解控制面通信机制

第 7 篇已经建立了 `Executor`、`Worker` 与 `ModelRunner` 的执行侧关系：`Scheduler` 产出 `SchedulerOutput`，`Executor` 把执行任务交给 worker，worker 内部的 `ModelRunner` 再把调度结果变成模型输入并返回 `ModelRunnerOutput`。

第 8 篇的核心问题更底层：**控制消息怎样跨对象、跨线程、跨进程、跨 worker 传递，又怎样把结果收回来**。如果把 vLLM 看成一条在线推理流水线，控制面负责决定请求何时进入、哪个执行单元要动作、哪些状态需要更新、结果从哪个出口回到前端。

控制面不是 TP / PP / DP 里搬运 GPU tensor 的高频数据面，也不是 KV connector 的跨节点 KV 传输。它关心的是**让谁做什么**和**由谁返回控制结果**：新增请求、终止请求、utility 调用、调度任务单、profile / sleep / wake、LoRA 操作、worker RPC、返回对象和异常状态，都属于这一层。

版本边界锚定 `code/opensource/vllm` 的本地源码快照，源码分支为 `main`，短提交哈希为 `52a31ccec`。源码位置只是事实边界，教程主线是建立一张可迁移的控制面地图：第一段是 Frontend / LLM / AsyncLLM 到 EngineCore，第二段是 EngineCore 到 Executor / Worker。

全局图用于先建立整体认知。蓝色箭头表示控制命令，绿色箭头表示控制结果，橙色虚线表示通信边界。

![vLLM 控制面通信的两段路径](imgs/08_control_plane_two_segments.png)

这张图可以分三层阅读。

第一层看**调用身份**。左侧的 Frontend、`LLMEngine`、`AsyncLLM` 或 API serving 代码，不直接操作 worker，它们只把用户请求加工成 `EngineCoreRequest`，再交给 `EngineCoreClient`。中间的 `EngineCore` 是调度与状态中心，它并不亲自跑模型，而是通过 `Executor` 把执行命令传给 worker group。右侧的 worker 才是设备进程或 actor，真正持有模型执行环境。

第二层看**通信边界**。前端到 EngineCore 的边界主要解决进程形态问题：EngineCore 可以在当前进程里，也可以在后台进程里，还可以出现在多 DP engine 或 headless engine 场景中。EngineCore 到 worker 的边界主要解决执行后端问题：后端可以是单进程 `UniProcExecutor`、本机多进程 `MultiprocExecutor`，也可以是 Ray actor 体系。

第三层看**返回路径**。绿色箭头虽然携带 `EngineCoreOutputs`、`ModelRunnerOutput`、utility result 等对象，但它仍然是控制面返回路径，不等价于模型执行数据面。数据面讨论的是 forward 期间的 TP collective、PP intermediate tensors、KV block transfer 等高频数据移动；控制面讨论的是 Python 对象、调度命令、worker 方法调用和结果收敛。

## 1. 控制面先拆成两段

vLLM 的控制面不是一条简单的函数调用链，而是两段性质不同的通信。

第一段是**客户端到 EngineCore**。上层可能是同步 `LLMEngine`，也可能是异步 `AsyncLLM`，还可能是 OpenAI API server 背后的 serving 对象。它们不应该关心 `EngineCore` 是否运行在同一进程、后台进程、多个 DP engine，还是远程 headless engine 里，所以 vLLM 把这层抽成 `EngineCoreClient`。
...
| --- | --- | --- | --- | --- |
| 客户端到引擎核心 | `LLMEngine` / `AsyncLLM` / API serving | `EngineCore` / `EngineCoreProc` | `ADD`、`ABORT`、`UTILITY`、`EngineCoreRequest`、`EngineCoreOutputs` | 前端不需要绑定 EngineCore 的进程形态、异步形态和 DP engine 形态； |
| 引擎核心到执行层 | `EngineCore` | `Executor` / `Worker` | `execute_model`、`sample_tokens`、utility RPC、`SchedulerOutput`、`ModelRunnerOutput` | EngineCore 不需要绑定 UniProc、Multiproc、Ray 等执行后端； |

把两段混在一起读，很容易产生两个误解。

第一个误解是**vLLM 的通信就是 ZMQ**。ZMQ 主要出现在多进程 `EngineCoreClient <-> EngineCoreProc` 这一段，也出现在 `MessageQueue` 的通知和溢出路径里；但 `InprocClient` 可以没有 ZMQ，`UniProcExecutor` 也可以直接本地调用 worker。

第二个误解是**只要看到返回对象，就是数据面**。`EngineCoreOutputs`、`ModelRunnerOutput`、utility result 这些返回对象仍然是控制面返回；真正的数据面要等进入模型 forward 后，才会出现 TP all-reduce、PP intermediate tensors、KV transfer 等路径。

第 7 篇与第 8 篇的关系可以这样定位：

1. 第 7 篇建立执行侧组件关系：谁负责调度、谁负责执行、谁负责设备进程、谁负责张量输入；
2. 第 8 篇下钻控制面通信：控制请求怎样进入 EngineCore，EngineCore 怎样把执行命令广播给 worker，结果怎样返回；
3. 后续数据面章节再解释模型执行中的 tensor、pipeline activation、KV cache 和跨节点传输怎样移动；

KV connector 不纳入这一章的主线。KV connector 本身同时包含控制消息和数据传输，但它依赖 `ModelRunner`、Attention、KV cache、block table 和 serving topology 等前置概念，提前放进控制面主线会让边界变得混乱。

## 2. EngineCoreClient 是第一段代理

上层引擎对象不会直接裸调用 `EngineCore`。同步路径里，`LLMEngine` 初始化 `InputProcessor` 和 `OutputProcessor`，然后通过 `EngineCoreClient.make_client()` 创建 engine core 客户端。`InputProcessor` 把上层输入转成 `EngineCoreRequest`，`OutputProcessor` 把 `EngineCoreOutputs` 转回用户可见的 `RequestOutput`。
...
这张图的重点不是哪一种更高级，而是三者把同一组语义映射到不同运行方式上。上层只需要面对 `add_request`、`abort_requests`、`get_output`、`call_utility` 这些语义，具体是本地方法调用、后台线程队列，还是 asyncio task，都被压在 client 抽象内部。

`InprocClient` 最容易理解。它在构造时直接创建 `EngineCore` 对象，`add_request()` 会先做 `preprocess_add_request()`，再调用 `engine_core.add_request()`；`get_output()` 则调用 `engine_core.step_fn()` 和 `post_step()`，最后取出 client 0 的 `EngineCoreOutputs`。这里没有进程边界，也没有 ZMQ，控制面退化为本地方法调用。

`SyncMPClient` 和 `AsyncMPClient` 属于多进程形态。它们继承自 `MPClient`，共同点是前台进程持有 input socket 和 output socket，后台 `EngineCoreProc` 运行核心循环；差异在于前台如何消费输出。同步客户端用后台线程把 ZMQ output socket 的内容塞进 `queue.Queue`，异步客户端用 asyncio task 把输出塞进 `asyncio.Queue`，这正好对应 `LLMEngine.step()` 和 `AsyncLLM` output handler 的不同消费方式。

从请求生命周期看，第一段代理可以拆成四步：

1. 输入对象先被 `InputProcessor` 规范化，形成 `EngineCoreRequest`；
2. `EngineCoreClient` 根据运行形态选择本地调用或 ZMQ 发送；
3. `EngineCore` 或 `EngineCoreProc` 接收 `ADD`、`ABORT`、`UTILITY` 等控制请求；
4. 输出对象经过 `EngineCoreOutputs` 回到 client，再由 `OutputProcessor` 转成前端可见结果；

这里的 client 不是业务客户端，而是 EngineCore 的本地代理。它的价值在于把**上层请求语义**和**EngineCore 部署形态**隔开，前端代码无需到处判断当前引擎是否多进程、是否异步、是否有多个 engine identity。

## 3. EngineCoreProc 的 ZMQ 消息边界

多进程模式下，前端进程和 `EngineCoreProc` 之间主要靠 ZMQ 与 msgpack 连接。ZMQ 负责把消息帧送过进程边界，msgpack 负责把 Python 控制对象编码成可传输的 payload，`EngineCoreProc` 则把 socket IO 与 EngineCore busy loop 分开。
...
3. 客户端发送 multipart message，形态是 `(engine_identity, request_type, serialized_request...)`；
4. `request_type` 是 `EngineCoreRequestType`，普通前端控制消息主要是 `ADD`、`ABORT`、`UTILITY`，DP 协调路径还会用到 `START_DP_WAVE`；
...
第一，`EngineCoreRequestType` 直接定义成 byte string。这样 request type 可以作为 socket frame 发出，不需要额外编码一层。`ADD` 是新增请求，`ABORT` 是终止请求，`UTILITY` 是调用 EngineCore 上的控制方法，`EXECUTOR_FAILED` 和 `WAKEUP` 更接近 `EngineCoreProc` 内部使用的哨兵，不应理解成普通客户端业务请求。
...
这个设计的收益是并发重叠。socket IO、msgpack 编解码、多模态 tensor IPC 与 GPU 执行不必完全串行排队；控制面本身不做 GPU 计算，但如果它阻塞在 socket 或编码上，在线推理的调度节奏也会被拖慢。
...
这仍然属于控制面，因为它发生在前端请求对象进入 EngineCore 的边界上，不是模型层里的 TP / PP tensor collective。这里的 tensor IPC 只是避免把请求携带的多模态 tensor 粗暴塞进 msgpack 主体。

第五，utility 调用复用同一条控制通道。`MPClient.call_utility()` 会生成 `call_id` 和 future，把 `(client_idx, call_id, method, args)` 作为 `UTILITY` 消息发给 `EngineCoreProc`。EngineCore 收到后用 `getattr(self, method_name)` 找到方法执行，再把 `UtilityOutput(call_id, result)` 放回 `output_queue`，客户端解码后用 `call_id` 找到 future，完成同步或异步等待。

因此，`reset_prefix_cache()`、`profile()`、`sleep()`、`wake_up()`、`get_supported_tasks()` 这些控制操作不需要另开一套 RPC 系统，它们与普通请求共享 EngineCore 控制入口，只是在返回侧通过 `call_id` 做精确匹配。

## 4. Executor 的 RPC 抽象

请求进入 `EngineCore` 后，控制面进入第二段：`EngineCore -> Executor -> Worker`。

`EngineCore.step()` 的关键路径可以压缩成下面的顺序：
...
这段流程里，`EngineCore` 不直接拿 worker 对象。它面对的是 `Executor`，而 `Executor` 抽象类把 worker 调用统一成 `collective_rpc()`。源码注释也明确提醒：这个 API 推荐只传控制消息，真正的数据面通信应该另行建立。
...
读 executor 源码时，更好的问题不是哪个类才是真正通信，而是**同一个控制语义，在不同后端里被映射成了什么通信机制**。只要这个问题抓稳，`UniProcExecutor`、`MultiprocExecutor`、Ray 后端就不再是三套互不相干的代码，而是同一条 EngineCore 控制语义的三个承载方式。

## 5. MessageQueue 与 worker busy loop

`MultiprocExecutor` 的核心难点是广播：leader 写一次控制命令，多个 worker 都要看到同一个 payload，而且 worker 可能分布在本地进程、远程 reader、不同 rank 上。

leader 进程初始化时，如果当前节点是 DP group leader，它会创建 `MessageQueue(world_size, local_world_size, ...)` 作为 `rpc_broadcast_mq`。每个 worker 进程初始化时会用 `MessageQueue.create_from_handle(handle, rank)` 连接同一个广播通道。

`MessageQueue` 不是普通 Python queue，它把本地读者和远程读者分开处理：
...
这解释了为什么 `MessageQueue` 比 `multiprocessing.Queue` 广播一下复杂得多。vLLM 要解决的是**一写多读**：leader 写入一份控制 payload，所有相关 worker 都要能读取，而且共享内存块不能在某个 worker 尚未读取时被覆盖。
...
源码里某些聚合场景会通过 `kv_output_aggregator` 影响返回策略。这里先把它当成**需要多个 worker 输出后再聚合的控制面收集策略**，不展开 KV 数据传输本身。

![MultiprocExecutor 的 MessageQueue 广播与回复机制](imgs/08_multiproc_message_queue.png)

worker 侧由 `worker_busy_loop()` 接住控制消息。它一直从 `rpc_broadcast_mq.dequeue(indefinite=True)` 取消息，然后执行下面的动作：
...
这一段最关键的结论是：**所有 worker 都会收到并执行控制命令，但不一定所有 worker 都向 leader 返回结果**。
...
理解了 worker 广播之后，`output_rank` 就不再是一个孤立参数。它描述的是**worker group 的返回收敛策略**，不是模型执行范围。

`output_rank` 的含义可以先用一句话记住：它不是只有这个 rank 执行模型，而是普通推理输出由哪个 rank 向上返回。

在 `MultiprocExecutor._get_output_rank()` 里，源码注释说明只从 TP rank 0 且最后一个 PP stage 的 worker 返回 `ModelRunnerOutput`。公式是：
...
图里的关键不是 rank 8 比其他 rank 更特殊，而是返回语义被压缩到了一个代表 rank 上。控制命令仍然广播给 worker group，模型执行所需的 TP / PP 协作仍然在数据面完成；到了 Python 控制面返回阶段，leader 只需要拿到能够驱动 Scheduler 更新状态的抽象输出。

这时最容易担心的是信息是否会丢失。答案是否定的，因为模型执行的数据面已经在 worker group 内部完成协作。TP rank 间的张量规约、PP stage 间的 intermediate tensors、最后 stage 的采样结果同步，都不是靠 leader 从所有 worker 收一遍 Python 对象来完成的。到了控制面返回阶段，`EngineCore` 需要的是本轮执行的抽象输出：哪些 request 产出了 token，哪些 request 完成，哪些状态要更新，这通常由最后输出 rank 代表上报即可。

`unique_reply_rank` 是 `collective_rpc()` 的参数语义：调用方告诉 executor，这次 RPC 只需要某个 rank 返回。`MultiprocExecutor` 内部把它转成 `output_rank` 写入广播 payload；worker busy loop 根据这个值决定自己是否写 response queue；leader 也据此只等待一个 response queue。

但不是所有控制面 RPC 都应该只等一个 rank。例如：
...
所以 `output_rank` 的本质不是优化掉其他 worker，而是**把返回收敛策略显式编码进控制面 payload**。下发仍然可以是广播，上报可以是单点、全量或聚合。

第 7 篇里所有 worker 执行和只收一个结果容易混淆，现在可以更精确地表达：
...
vLLM V1 的控制面主链路可以从一次新增请求串起来。

请求先在前端被加工成 `EngineCoreRequest`。如果是 `InprocClient`，它直接进入当前进程里的 `EngineCore`；如果是 `SyncMPClient` 或 `AsyncMPClient`，它会被 msgpack 编码，通过 ZMQ ROUTER / DEALER 进入后台 `EngineCoreProc`，再由 input socket 线程放入 `input_queue`。EngineCore busy loop 处理 `ADD`、`ABORT`、`UTILITY` 等请求，并在有调度工作时调用 `step_fn()`。
...
这一章没有展开数据面，不是因为数据面不重要，而是因为它需要更多前置知识。更合理的后续顺序是：

1. `ModelRunner` 如何把 `SchedulerOutput` 变成 input ids、positions、block table、attention metadata 和 sampling metadata；
2. Attention 与 KV cache 的执行结构；
3. Sampler 与 OutputProcessor 的返回路径；
4. TP / PP / DP 的模型执行数据面通信；
5. KV cache、prefix cache、disaggregated prefill 和 serving topology 铺好后，再进入 KV connector 与跨节点 KV transfer；
...
1. 单选：本章把 vLLM 控制面通信拆成两段，最准确的是哪一项？
...
6. 单选：`Executor.collective_rpc()` 在本章中的定位是什么？
...
12. 多选：关于 `output_rank` 和 `unique_reply_rank`，哪些理解是正确的？
    A. `unique_reply_rank` 是调用侧表达只需要某个 rank 回复的参数语义；
...
1. B。本章的两段控制面是前端到 EngineCore、EngineCore 到 Worker。C 和 D 属于后续数据面或存储 / 传输主题；
...
7. A、B、D。Multiproc 广播 payload 会包含方法、参数和返回策略；callable 可能需要先被 `cloudpickle` 序列化。C 错在把谁回复误解成谁收到广播，worker 仍然可以都收到并执行控制命令；
...
12. A、B、D。C 错在把谁回复误解成谁收到命令。下发可以是所有 worker，返回可以只收一个 rank；
