...
前面几篇我们已经把 `Executor`、`Worker` 与 `ModelRunner` 的协同关系讲清楚了：`Scheduler` 产出 `SchedulerOutput`，`Executor` 把执行任务交给 worker，worker 内部的 `ModelRunner` 再把调度结果变成模型输入并返回 `ModelRunnerOutput`。
